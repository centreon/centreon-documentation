...
Long TTFB usually indicates the application isn't using a full-page cache (for example, Varnish or other full-page caches). In that case the delay stems from the time the application needs to generate the page. This also consumes server resources (typically CPU), which can impact:
