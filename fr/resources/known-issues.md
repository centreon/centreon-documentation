...
|L'Autologin ne fonctionne pas avec certaines pages| Actuellement, la connexion automatique ne fonctionne pas sur les pages suivantes : <ul><li>Monitoring > Resources Status</li><li>Configuration > Hosts > Discovery</li><li>Configuration > Business Activity > Business Views</li><li>Configuration > Business Activity > BBusiness Activity</li></ul>|
| Le contenu des pages n'est pas traduit selon la langue de mon utilisateur | Vous devez installer les langues sur votre système d'exploitation avec la commande suivante : <br /> `yum install -y glibc-all-langpacks` <br /> puis redémarrez PHP à l'aide de la commande suivante : <br /> `systemctl restart php-fpm` |