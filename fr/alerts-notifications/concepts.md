...
[Statut des ressources](resources-status.html). Vous pouvez filtrer cette page en fonction des statuts et de certains états.
...
- <span style="color:#ae9500">Acquitté</span> : indique que l'incident sur le service ou l'hôte est pris en
    compte par un utilisateur. (Voir [Acquitter un problème](acknowledge.html).)
- <span style="color:#cc99ff">En maintenance</span> : indique que les notifications sont temporairement suspendues pour cette ressource. Une [plage de maintenance](downtimes.html) peut être [planifiée à l'avance](downtimes.html#les-temps-darrêt-récurrents) pour éviter de recevoir des alertes pendant une opération de maintenance. Elle peut également être définie suite à un incident.
- [Flapping](notif-flapping.html) (bagotement) : le pourcentage de
...
Vous pouvez filtrer la page [Resources Status](resources-status.html) suivant le type de statut.
