...
Configuring an OpenTelemetry Collector can be complex. Log Management provides an interface where you can select configuration snippets for your data sources, edit them to match your setup, and download the ready-to-use result. More data sources will be added over time. If your data source is not listed, use the [manual configuration procedure](collector.md) instead.
...
2. In the window that opens, select the correct pack for the data source you want. (At the moment, a pack contains just one snippet):
   * Use the search bar at the top of the window, or
   * Click **Add pack** in the **Configure packs** section.
   Once you have selected a pack, a summary appears in the **Configure packs** section, and the **Preview configuration YAML** panel is updated: your data source is added to the **receivers** section of the configuration file.
3. Check if the log file displayed in the **Configure packs** section for your data source is correct.
...
2. Check that everything is correct in the **Preview configuration YAML** panel.
   * An exporter for Centreon Log Management is already pre-configured (you will have to fill it out at [step 3](#step-3-enter-the-authentication-token-for-centreon-log-management)).
