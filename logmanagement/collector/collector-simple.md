...
2. In Centreon Log Management, [generate a token to authenticate the host to your CLM instance](../administration/tokens.md).
...
4. Replace the file's contents by the snippet below. Make sure you save the file as an administrator.
...
In the following example, we receive data from three different sources on the same Windows server. Logs go through only one pipeline. All of the configuration is gathered into the collector's **config.yaml** file. (Follow the same procedure as above and adapt the configuration file below.)
