...
You can delete a token using the **Delete** icon on the **Administration > Token management** page. If you delete a token, any hosts that are using it will no longer be able to authenticate with CLM. As a result, logs sent by those hosts will stop reaching your CLM instance until a new valid token is configured. Deleting a token cannot be undone.
