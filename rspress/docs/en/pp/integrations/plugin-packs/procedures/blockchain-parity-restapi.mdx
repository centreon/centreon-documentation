...
The following monitoring connectors will be installed when you install the **Parity API** connector through the
...
The Monitoring Connector **Parity API** brings a host template:
...
<Tabs groupId="sync">
<Tab value="Blockchain-Parity-Restapi-custom" label="Blockchain-Parity-Restapi-custom">
...
2. Whatever the license type (*online* or *offline*), install the **Parity API** connector through
...
3. Apply the **Blockchain-Parity-Restapi-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
...
| PARITYEXTRAOPTIONS | Any extra option you may want to add to every command (a --verbose flag for example). All options are listed [here](#available-options). |                   |             |
