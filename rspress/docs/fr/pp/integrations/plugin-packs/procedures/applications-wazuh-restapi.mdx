...
Le connecteur de supervision **Wazuh Rest API** apporte un modèle d'hôte :
...
<Tabs groupId="sync">
<Tab value="App-Wazuh-Restapi-custom" label="App-Wazuh-Restapi-custom">

| Alias   | Modèle de service                | Description                                   |
|:--------|:---------------------------------|:----------------------------------------------|
| Agents  | App-Wazuh-Agents-Restapi-custom  | Contrôle le statut des agents                 |
| Manager | App-Wazuh-Manager-Restapi-custom | Contrôle les processus et les logs du manager |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **App-Wazuh-Restapi-custom** est utilisé.
...
Se référer à la [documentation officielle de Wazuh](https://documentation.wazuh.com/) pour la création d'un utilisateur permettant d'interroger l'API.
...
3. Appliquez le modèle d'hôte **App-Wazuh-Restapi-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
| WAZUHAPIEXTRAOPTIONS | Any extra option you may want to add to every command (e.g. a --verbose flag). Toutes les options sont listées [ici](#options-disponibles) |                   |             |
