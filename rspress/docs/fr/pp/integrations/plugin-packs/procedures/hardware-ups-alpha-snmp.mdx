...
Le connecteur de supervision **Alpha UPS SNMP** apporte un modèle d'hôte :

* **HW-UPS-Alpha-SNMP-custom**

Le connecteur apporte les modèles de service suivants
(classés selon le modèle d'hôte auquel ils sont rattachés) :

<Tabs groupId="sync">
<Tab value="HW-UPS-Alpha-SNMP-custom" label="HW-UPS-Alpha-SNMP-custom">

| Alias          | Modèle de service                       | Description                    |
|:---------------|:----------------------------------------|:-------------------------------|
| Alarms         | HW-UPS-Alpha-Alarms-SNMP-custom         | Contrôle les alarmes           |
| Battery-Status | HW-UPS-Alpha-Battery-Status-SNMP-custom | Contrôle l'état de la batterie |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **HW-UPS-Alpha-SNMP-custom** est utilisé.
...
|:----------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SNMP Agents     | Découvre les ressources via un scan réseau SNMP. Installez le connecteur [Generic SNMP](./applications-protocol-snmp.md) pour obtenir la règle de découverte et créez un modificateur pour le modèle d'hôte **HW-UPS-Alpha-SNMP-custom**. |
...
```bash
dnf install centreon-pack-hardware-ups-alpha-snmp
```
...
```bash
apt install centreon-pack-hardware-ups-alpha-snmp
```
...
```bash
yum install centreon-pack-hardware-ups-alpha-snmp
```

</Tab>
</Tabs>

2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Alpha UPS SNMP**
...
```bash
dnf install centreon-plugin-Hardware-Ups-Alpha-Snmp
```
...
```bash
apt install centreon-plugin-hardware-ups-alpha-snmp
```
...
3. Appliquez le modèle d'hôte **HW-UPS-Alpha-SNMP-custom**.
...
```bash
/usr/lib/centreon/plugins/centreon_ups_alpha_snmp.pl \
	--plugin=hardware::ups::alpha::snmp::plugin \
	--mode=battery-status \
	--hostname=10.0.0.1 \
	--snmp-version='2c' \
...
```bash
OK: Remaining capacity : 22 % Current : 98 A Voltage : 33 V Temperature : 93 C | 'status'=88;;;;'load'=22%;;;0;100'current'=98A;;;0;'voltage'=33V;;;;'temperature'=93C;;;;
```
...
|:-------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------|
| alarms [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/hardware/ups/alpha/snmp/mode/alarms.pm)]                | HW-UPS-Alpha-Alarms-SNMP-custom         |
| battery-status [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/hardware/ups/alpha/snmp/mode/batterystatus.pm)] | HW-UPS-Alpha-Battery-Status-SNMP-custom |
...
| Option               | Description                                                                                                                                                                                                             |
|:---------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| --filter             | Exclude some parts (comma separated list) You can also exclude items from specific instances: --filter="alarm,FAN Alarm"                                                                                                |
| --no-component       | Define the expected status if no components are found (default: critical).                                                                                                                                              |
| --threshold-overload | Use this option to override the status returned by the plugin when the status label matches a regular expression (syntax: section,\[instance,\]status,regexp). Example: --threshold-overload='alarm,FAN Alarm,OK,on'    |
