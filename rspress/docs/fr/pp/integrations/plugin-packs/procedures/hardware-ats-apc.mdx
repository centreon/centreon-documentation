...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **APC ATS SNMP** 
...
<Tabs groupId="sync">
<Tab value="HW-ATS-Apc-SNMP-custom" label="HW-ATS-Apc-SNMP-custom">

| Alias         | Modèle de service                    | Description                      |
|:--------------|:-------------------------------------|:---------------------------------|
| Device-Status | HW-ATS-Apc-Device-Status-SNMP-custom | Contrôle l'état de l'équipement  |
| Input-Lines   | HW-ATS-Apc-Input-Lines-SNMP-custom   | Contrôle les métriques en entrée |
| Output-Lines  | HW-ATS-Apc-Output-Lines-SNMP-custom  | Contrôle les métriques en sortie |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **HW-ATS-Apc-SNMP-custom** est utilisé.
...
<Tabs groupId="sync">
<Tab value="Device-Status" label="Device-Status">

| Nom                   | Unité |
|:----------------------|:------|
| hardware.entity.count | count |
| entity status         | N/A   |
...
```bash
dnf install centreon-pack-hardware-ats-apc
```
...
```bash
dnf install centreon-plugin-Hardware-Ats-Apc-Snmp
```
...
3. Appliquez le modèle d'hôte **HW-ATS-Apc-SNMP-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
| WARNINGLOADCAPACITY  | Threshold                                                                                                                                        |                                        |             |
| CRITICALLOADCAPACITY | Threshold                                                                                                                                        |                                        |             |
| WARNINGPOWER         | Threshold                                                                                                                                        |                                        |             |
| CRITICALPOWER        | Threshold                                                                                                                                        |                                        |             |
| WARNINGSTATUS        | Define the conditions to match for the status to be WARNING. You can use the following variables: %\{status\}, %\{display\}                      | %\{status\} =~ /nearoverload/          |             |
| CRITICALSTATUS       | Define the conditions to match for the status to be CRITICAL$/'). You can use the following variables: %\{status\}, %\{display\}                 | %\{status\} =~ /^(lowload\|overload)$/ |             |
| WARNINGVOLTAGE       | Threshold                                                                                                                                        |                                        |             |
| CRITICALVOLTAGE      | Threshold                                                                                                                                        |                                        |             |
| EXTRAOPTIONS         | Any extra option you may want to add to the command (a --verbose flag for example). Toutes les options sont listées [ici](#options-disponibles). | --verbose                              |             |
...
/usr/lib/centreon/plugins/centreon_ats_apc_snmp.pl \
	...
	--warning-status='%\{status\} =~ /nearoverload/' \
	--critical-status='%\{status\} =~ /^(lowload|overload)$/' \
	--warning-voltage='' \
	--critical-voltage=''  \
	--warning-current='' \
	--critical-current=''  \
...
|:---------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------|
| device-status [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/hardware/ats/apc/snmp/mode/devicestatus.pm)] | HW-ATS-Apc-Device-Status-SNMP-custom          |
| input-lines [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/hardware/ats/apc/snmp/mode/inputlines.pm)]     | HW-ATS-Apc-Input-Lines-SNMP-custom            |
| output-lines [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/hardware/ats/apc/snmp/mode/outputlines.pm)]   | HW-ATS-Apc-Output-Lines-SNMP-custom           |
| time [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/hardware/ats/apc/snmp/mode/ntp.pm)]                   | Non utilisé dans ce connecteur de supervision |
...
| --filter             | Exclude the items given as a comma-separated list (example: --filter=entity). You can also exclude items from specific instances: --filter=entity,CommStatus                                                                     |
...
| --threshold-overload | Use this option to override the status returned by the plugin when the status label matches a regular expression (syntax: section,\[instance,\]status,regexp). Example: --threshold-overload='entity,SourceBStatus,WARNING,fail' |
