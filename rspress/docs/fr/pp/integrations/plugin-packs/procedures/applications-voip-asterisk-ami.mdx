...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Asterisk VoIP AMI**
...
Le connecteur de supervision **Asterisk VoIP AMI** apporte un modèle d'hôte :
...
<Tabs groupId="sync">
<Tab value="App-VoIP-Asterisk-AMI-custom" label="App-VoIP-Asterisk-AMI-custom">
...
| Alias        | Modèle de service                         | Description                           |
|:-------------|:------------------------------------------|:--------------------------------------|
| Dahdi-Status | App-Voip-Asterisk-AMI-Dahdi-Status-custom | Contrôle le statut des lignes 'dahdi' |
...
<Tabs groupId="sync">
<Tab value="Channel-Usage" label="Channel-Usage">
...
<Tab value="Dahdi-Status" label="Dahdi-Status">
...
<Tab value="Sip-Peers-Usage" label="Sip-Peers-Usage">
...
2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Asterisk VoIP AMI**
...
3. Appliquez le modèle d'hôte **App-VoIP-Asterisk-AMI-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
| ASTERISKAMIEXTRAOPTIONS | Any extra option you may want to add to every command (a --verbose flag for example). Toutes les options sont listées [ici](#options-disponibles). |                   |             |
