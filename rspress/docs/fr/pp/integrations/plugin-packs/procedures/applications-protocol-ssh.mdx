...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Protocol SSH**
...
Le connecteur de supervision **Protocol SSH** apporte un modèle d'hôte :

* **App-Protocol-Ssh-custom**

Le connecteur apporte le modèle de service suivant
(classé selon le modèle d'hôte auquel il est rattaché) :

<Tabs groupId="sync">
<Tab value="App-Protocol-Ssh-custom" label="App-Protocol-Ssh-custom">

| Alias     | Modèle de service             | Description                          |
|:----------|:------------------------------|:-------------------------------------|
| Ssh-Login | App-Protocol-Ssh-Login-custom | Contrôle la connexion SSH à distance |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **App-Protocol-Ssh-custom** est utilisé.
...
<Tabs groupId="sync">
<Tab value="Ssh-Login" label="Ssh-Login">
...
```bash
dnf install centreon-pack-applications-protocol-ssh
```
...
```bash
apt install centreon-pack-applications-protocol-ssh
```
...
2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Protocol SSH**
...
```bash
dnf install centreon-plugin-Applications-Protocol-Ssh
```
...
```bash
apt install centreon-plugin-applications-protocol-ssh
```
...
3. Appliquez le modèle d'hôte **App-Protocol-Ssh-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
4. Renseignez les macros désirées. Attention, certaines macros sont obligatoires.

| Macro                   | Description                                                                                          | Valeur par défaut | Obligatoire |
|:------------------------|:-----------------------------------------------------------------------------------------------------|:------------------|:-----------:|
| PROTOCOLSSHUSERNAME     | SSH username                                                                                         |                   | X            |
| PROTOCOLSSHPASSWORD     | SSH password                                                                                         |                   | X            |
| PROTOCOLSSHEXTRAOPTIONS | Any extra option you may want to add to every command (a --verbose flag for example). Toutes les options sont listées [ici](#options-disponibles). |                   |             |
...
| Macro          | Description                                                                                                                                                                         | Valeur par défaut                           | Obligatoire |
|:---------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------|:-----------:|
| CRITICALSTATUS | Define the conditions to match for the status to be CRITICAL (default: '%\{message\} !~ /authentication succeeded/i' You can use the following variables: %\{status\}, %\{message\} | %\{message\} !~ /authentication succeeded/i |             |
...
/usr/lib/centreon/plugins/centreon_protocol_ssh.pl \
	...
	--ssh-password='xxxxxx' \
	--warning-status='' \
	--critical-status='%\{message\} !~ /authentication succeeded/i' \
	--warning-time='' \
	--critical-time=''
```
...
```bash
OK: Response time 7957 | 'response.time.seconds'=7957s;;;0;
```
...
|:---------------------------------------------------------------------------------------------------------------|:------------------------------|
| login [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/protocols/ssh/mode/login.pm)] | App-Protocol-Ssh-Login-custom |
