...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Buffalo TeraStation SNMP**
...
Le connecteur de supervision **Buffalo TeraStation SNMP** apporte un modèle d'hôte :

* **HW-Storage-Buffalo-Terastation-SNMP-custom**

Le connecteur apporte les modèles de service suivants
(classés selon le modèle d'hôte auquel ils sont rattachés) :

<Tabs groupId="sync">
<Tab value="HW-Storage-Buffalo-Terastation-SNMP-custom" label="HW-Storage-Buffalo-Terastation-SNMP-custom">

| Alias           | Modèle de service                                          | Description                                                                                                                                                                        |
|:----------------|:-----------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Array-Usage     | HW-Storage-Buffalo-Terastation-Array-Usage-SNMP-custom     | Contrôle l'utilisation des disques                                                                                                                                                 |
| Cpu-Detailed    | HW-Storage-Buffalo-Terastation-Cpu-Detailed-SNMP-custom    | Contrôle du taux d'utilisation détaillé CPU de la machine. Ce contrôle pourra remonter la moyenne du taux d'utilisation des CPU ainsi que le taux par CPU pour les CPU multi-coeur |
| Hardware-Global | HW-Storage-Buffalo-Terastation-Hardware-Global-SNMP-custom | Contrôle le matériel                                                                                                                                                               |
| Load            | HW-Storage-Buffalo-Terastation-Load-SNMP-custom            | Contrôle de la charge serveur                                                                                                                                                      |
| Memory          | HW-Storage-Buffalo-Terastation-Memory-SNMP-custom          | Contrôle du taux d'utilisation de la mémoire vive (RAM)                                                                                                                            |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **HW-Storage-Buffalo-Terastation-SNMP-custom** est utilisé.
...
| Alias      | Modèle de service                                     | Description             |
|:-----------|:------------------------------------------------------|:------------------------|
| Interfaces | HW-Storage-Buffalo-Terastation-Interfaces-SNMP-custom | Contrôle les interfaces |
...
|:----------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SNMP Agents     | Découvre les ressources via un scan réseau SNMP. Installez le connecteur [Generic SNMP](./applications-protocol-snmp.md) pour obtenir la règle de découverte et créez un modificateur pour le modèle d'hôte **HW-Storage-Buffalo-Terastation-SNMP-custom**. |
...
<Tabs groupId="sync">
<Tab value="Array-Usage" label="Array-Usage">

| Nom                             | Unité |
|:--------------------------------|:------|
| *array*#array.space.usage.bytes | B     |
...
```bash
dnf install centreon-pack-hardware-storage-buffalo-terastation-snmp
```
...
```bash
apt install centreon-pack-hardware-storage-buffalo-terastation-snmp
```
...
```bash
yum install centreon-pack-hardware-storage-buffalo-terastation-snmp
```

</Tab>
</Tabs>

2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Buffalo TeraStation SNMP**
...
```bash
dnf install centreon-plugin-Hardware-Storage-Buffalo-Terastation-Snmp
```
...
```bash
apt install centreon-plugin-hardware-storage-buffalo-terastation-snmp
```
...
3. Appliquez le modèle d'hôte **HW-Storage-Buffalo-Terastation-SNMP-custom**.
...
| Macro        | Description                                                                                        | Valeur par défaut | Obligatoire |
|:-------------|:---------------------------------------------------------------------------------------------------|:------------------|:-----------:|
| COMPONENT    | Which component to check (default: '.*'). Can be: 'disk', 'iscsi', 'psu'                           | .*                |             |
...
```bash
/usr/lib/centreon/plugins/centreon_buffalo_terastation_snmp.pl \
	--plugin=storage::buffalo::terastation::snmp::plugin \
	--mode=interfaces \
	--hostname=10.0.0.1 \
	--snmp-version='2c' \
	...
	--add-errors \
	--warning-status='' \
	--critical-status='%\{admstatus\} eq "up" and %\{opstatus\} !~ /up/' \
	--warning-in-traffic='' \
	--critical-in-traffic='' \
	--warning-out-traffic='' \
...
```bash
OK: All interfaces are ok | 'interface_name1#interface.traffic.in.bitspersecond'=96689b/s;;;; 'interface_name2#interface.traffic.in.bitspersecond'=20527b/s;;;; 'interface_name1#interface.traffic.out.bitspersecond'=29920b/s;;;; 'interface_name2#interface.traffic.out.bitspersecond'=32529b/s;;;; 'interface_name1#interface.packets.in.discard.percentage'=88%;;;;100 'interface_name2#interface.packets.in.discard.percentage'=57%;;;;100 'interface_name1#interface.packets.in.error.percentage'=22%;;;;100 'interface_name2#interface.packets.in.error.percentage'=78%;;;;100 'interface_name1#interface.packets.out.discard.percentage'=26%;;;;100 'interface_name2#interface.packets.out.discard.percentage'=7%;;;;100 'interface_name1#interface.packets.out.error.percentage'=51%;;;;100 'interface_name2#interface.packets.out.error.percentage'=17%;;;;100
```
...
|:----------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------|
| array-usage [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/storage/buffalo/terastation/snmp/mode/arrayusage.pm)] | HW-Storage-Buffalo-Terastation-Array-Usage-SNMP-custom     |
| cpu-detailed [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/snmp_standard/mode/cpudetailed.pm)]                  | HW-Storage-Buffalo-Terastation-Cpu-Detailed-SNMP-custom    |
| hardware [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/storage/buffalo/terastation/snmp/mode/hardware.pm)]      | HW-Storage-Buffalo-Terastation-Hardware-Global-SNMP-custom |
| interfaces [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/snmp_standard/mode/interfaces.pm)]                     | HW-Storage-Buffalo-Terastation-Interfaces-SNMP-custom      |
...
| load [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/snmp_standard/mode/loadaverage.pm)]                          | HW-Storage-Buffalo-Terastation-Load-SNMP-custom            |
| memory [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/snmp_standard/mode/memory.pm)]                             | HW-Storage-Buffalo-Terastation-Memory-SNMP-custom          |
...
|:---------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| --component          |   Which component to check (default: '.*'). Can be: 'disk', 'iscsi', 'psu'.                                                                                                                                              |
| --filter             |   Exclude the items given as a comma-separated list (example: --filter=disk --filter=psu). You can also exclude items from specific instances: --filter=psu,1                                                            |
...
| --threshold-overload |   Use this option to override the status returned by the plugin when the status label matches a regular expression (syntax: section,status,regexp). Example: --threshold-overload='disk,OK,^(?!(degrade)$)'              |
