...
The following monitoring connectors will be installed when you install the **Linux Libvirt SSH** connector through the
...
The Monitoring Connector **Linux Libvirt SSH** brings 2 host templates:
...
|:--------------|:-----------------------------------------------------|:--------------------------------------------|:---------:|
| Pool-Status   | Virt-Linux-Libvirt-Hypervisor-Pool-Status-SSH-custom | Check libvirt storage pool status and usage | X         |
| Volume        | Virt-Linux-Libvirt-Hypervisor-Volume-SSH-custom      | Check libvirt storage volume allocation     | X         |

> The services listed above are created automatically when the **Virt-Linux-Libvirt-Hypervisor-SSH-custom** host template is used.
...
|:--------------|:-----------------------------------------|:----------------------------------------------------|
| Cpu           | Virt-Linux-Libvirt-Vm-Cpu-SSH-custom     | Check virtual machines CPU usage                    |
| Disk-Io       | Virt-Linux-Libvirt-Vm-Disk-Io-SSH-custom | Check virtual machines disk I/O statistics          |
| Memory        | Virt-Linux-Libvirt-Vm-Memory-SSH-custom  | Check virtual machines memory usage                 |
| Network       | Virt-Linux-Libvirt-Vm-Network-SSH-custom | Check virtual machines network interface statistics |
| Status        | Virt-Linux-Libvirt-Vm-Status-SSH-custom  | Check virtual machines status                       |

> The services listed above are created automatically when the **Virt-Linux-Libvirt-VM-SSH-custom** host template is used.
...
| Rule name                                     | Description |
|:----------------------------------------------|:------------|
| Virt-Linux-Libvirt-Hypervisor-SSH-Pools-Name  | Discover libvirt storage pool       |
| Virt-Linux-Libvirt-Hypervisor-SSH-Volume-Name | Discover libvirt storage volume     |
...
Using this connector requires creating a user on the monitored resource, which 
will be used by the Centreon poller to authenticate and execute SSH requests. 
No sudo or root privileges are required; a 'simple' user is enough.

Two SSH connection methods are possible:
* exchanging the public SSH key of the **centreon-engine** user of the Centreon poller (recommended)
* defining your user and password directly in the host macros.

You must also configure the connection with the **centreon-gorgone** user if you intend
to use the service discovery rules of this connector.
...
2. Whatever the license type (*online* or *offline*), install the **Linux Libvirt SSH** connector through
...
3. Apply the **Virt-Linux-Libvirt-Hypervisor-SSH-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
...
1. Log into Centreon and add a new host through **Configuration > Hosts**.
2. Fill in the **Name**, **Alias** & **IP Address/DNS** fields according to your resource's settings.
3. Apply the **Virt-Linux-Libvirt-VM-SSH-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
4. Fill in the macros you want. Some macros are mandatory.
...
|:---------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------|
| discovery [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/cloud/linux/libvirt/local/mode/discovery.pm)]    | Used for host discovery                              |
