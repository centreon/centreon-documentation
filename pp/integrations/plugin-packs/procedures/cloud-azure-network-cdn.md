...
The Monitoring Connector **Azure CDN** brings a host template:

* **Cloud-Azure-Network-CDN-custom**

The connector brings the following service templates (sorted by the host template they are attached to):

<Tabs groupId="sync">
<TabItem value="Cloud-Azure-Network-CDN-custom" label="Cloud-Azure-Network-CDN-custom">

| Service Alias | Service Template                                 | Service Description                                   |
|:--------------|:-------------------------------------------------|:------------------------------------------------------|
| Hit-Ratio     | Cloud-Azure-Network-CDN-Hit-Ratio-Api-custom     | Check Azure CDN profile service bytes hit ratio       |
| Latency       | Cloud-Azure-Network-CDN-Latency-Api-custom       | Check Azure CDN profile service HTTP requests latency |
| Requests      | Cloud-Azure-Network-CDN-Requests-Api-custom      | Check Azure CDN profile service HTTP requests count   |
| Response-Size | Cloud-Azure-Network-CDN-Response-Size-Api-custom | Check Azure CDN profile service HTTP response size    |

> The services listed above are created automatically when the **Cloud-Azure-Network-CDN-custom** host template is used.
...
to the list of monitored hosts. This provider is named **Microsoft Azure CDN**.
...
| Name                           | Unit  |
|:-------------------------------|:------|
| cdn.latency.total.milliseconds | ms    |
...
```bash
dnf install centreon-pack-cloud-azure-network-cdn
```
...
```bash
apt install centreon-pack-cloud-azure-network-cdn
```
...
2. Whatever the license type (*online* or *offline*), install the **Azure CDN** connector through
...
```bash
apt install centreon-plugin-cloud-azure-network-cdn-api
```
...
3. Apply the **Cloud-Azure-Network-CDN-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
...
| WARNINGBYTEHITPERCENTAGE  | Warning threshold                                                                                                                                  |                   |             |
| CRITICALBYTEHITPERCENTAGE | Critical threshold                                                                                                                                 |                   |             |
...
| FILTERMETRIC         | Filter metrics                                                                                                                                                   |                   |             |
| FILTERDIMENSION      | Specify the metric dimension (required for some specific metrics) Syntax example: --filter-dimension="$metricname eq '$metricvalue'"               |                   |             |
| WARNINGTOTALLATENCY  | Warning threshold                                                                                                                                  |                   |             |
| CRITICALTOTALLATENCY | Critical threshold                                                                                                                                 |                   |             |
| EXTRAOPTIONS         | Any extra option you may want to add to the command (a --verbose flag for example). All options are listed [here](#available-options).                                                 |                   |             |
...
/usr/lib/centreon/plugins/centreon_azure_network_cdn_api.pl  \
	...
	--critical-response-size=''
```

The expected command output is shown below:

```bash
OK: Response Size | 'cdn.response.size.bytes'=1950B;;;0;
```
...
|:----------------------------------------------------------------------------------------------------------------------------|:-------------------------------------------------|
| discovery [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/cloud/azure/network/cdn/mode/discovery.pm)] | Used for host discovery                          |
| hit-ratio [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/cloud/azure/network/cdn/mode/hitratio.pm)]  | Cloud-Azure-Network-CDN-Hit-Ratio-Api-custom     |
| latency [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/cloud/azure/network/cdn/mode/latency.pm)]     | Cloud-Azure-Network-CDN-Latency-Api-custom       |
| requests [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/cloud/azure/network/cdn/mode/requests.pm)]   | Cloud-Azure-Network-CDN-Requests-Api-custom      |
| response [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/cloud/azure/network/cdn/mode/response.pm)]   | Cloud-Azure-Network-CDN-Response-Size-Api-custom |
...
| --warning-byte-hit-percentage  |   Warning threshold.                                                                                                          |
| --critical-byte-hit-percentage |   Critical threshold.                                                                                                         |
...
| --resource               |   Set resource name or ID (required).                         |
| --resource-group         |   Set resource group (required if resource's name is used).   |
| --warning-total-latency  |   Warning threshold.                                          |
| --critical-total-latency |   Critical threshold.                                         |
...
| --resource       |   Set resource name or ID (required).                                                                                                         |
| --resource-group |   Set resource group (required if resource's name is used).                                                                                   |
| --warning-*      |   Warning threshold where '*' can be: 'requests-count', '4xx-requests-percentage',, '5xx-requests-percentage', 'firewall-requests-count'.     |
| --critical-*     |   Critical threshold where '*' can be: 'requests-count', '4xx-requests-percentage',, '5xx-requests-percentage', 'firewall-requests-count'.    |
...
| --resource       |   Set resource name or ID (required).                         |
| --resource-group |   Set resource group (required if resource's name is used).   |
| --warning-*      |   Warning threshold where '*' can be: 'response-size'.        |
| --critical-*     |   Critical threshold where '*' can be: 'response-size'.       |
