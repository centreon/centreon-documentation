...
The Monitoring Connector **Veritas Backup Exec NSCP Rest API** brings a host template:
...
> The services listed above are created automatically when the **App-Backupexec-Nscp-Restapi-custom** host template is used.
...
2. Whatever the license type (*online* or *offline*), install the **Veritas Backup Exec NSCP Rest API** connector through
...
| NSCPRESTAPIEXTRAOPTIONS   | Any extra option you may want to add to every command (E.g. a --verbose flag). All options are listed [here](#available-options) | --http-backend=curl --insecure                             |           |
