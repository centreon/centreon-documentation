...
description: "Monitor Microsoft WSUS servers via NRPE or the NSClient++ REST API: computer status, server statistics, synchronization, and update status."
---
...
The following monitoring connectors will be installed when you install the **Microsoft WSUS** connector through the
...
The Monitoring Connector **Microsoft WSUS** brings 2 host templates:
...
> Applies to the following service templates: Update-Status, Update-Status
...
2. Whatever the license type (*online* or *offline*), install the **Microsoft WSUS** connector through
...
3. Apply the **App-Wsus-NRPE-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
...
| NRPEEXTRAOPTIONS          | Any extra option you may want to add to every command (a --verbose flag for example). Toutes les options sont listées [ici](#available-options). |                       |             |
...
1. Log into Centreon and add a new host through **Configuration > Hosts**.
2. Fill in the **Name**, **Alias** & **IP Address/DNS** fields according to your resource's settings.
3. Apply the **App-Wsus-NSClient-05-Restapi-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
4. Fill in the macros you want. Some macros are mandatory.
