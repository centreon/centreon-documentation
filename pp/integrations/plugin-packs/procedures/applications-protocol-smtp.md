...
The Monitoring Connector **SMTP Server** brings a host template:
...
|:--------------|:---------------------------------|:-----------------------------------------|
| SMTP-Login    | App-Protocol-SMTP-Login-custom   | Check connection to a SMTP server        |
| SMTP-Message  | App-Protocol-SMTP-Message-custom | Check sending a message to a SMTP server |

> The services listed above are created automatically when the **App-Protocol-SMTP-custom** host template is used.
...
2. Whatever the license type (*online* or *offline*), install the **SMTP Server** connector through
...
3. Apply the **App-Protocol-SMTP-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
...
| SMTPEXTRAOPTIONS | Any extra option you may want to add to every command (a --verbose flag for example). All options are listed [here](#available-options). |                   |             |
