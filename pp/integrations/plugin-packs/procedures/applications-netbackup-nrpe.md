...
The following monitoring connectors will be installed when you install the **Symantec Netbackup** connector through the
...
The Monitoring Connector **Symantec Netbackup** brings a host template:

* **App-Netbackup-NRPE-custom**

The connector brings the following service templates (sorted by the host template they are attached to):

<Tabs groupId="sync">
<TabItem value="App-Netbackup-NRPE-custom" label="App-Netbackup-NRPE-custom">

| Service Alias  | Service Template                         | Service Description        |
|:---------------|:-----------------------------------------|:---------------------------|
| Dedup-Status   | App-Netbackup-Dedup-Status-NRPE-custom   | Check deduplication status |
| Drive-Cleaning | App-Netbackup-Drive-Cleaning-NRPE-custom | Check drive cleaning       |
| Drive-Status   | App-Netbackup-Drive-Status-NRPE-custom   | Check drive status         |
| Job-Status     | App-Netbackup-Job-Status-NRPE-custom     | Check job status           |

> The services listed above are created automatically when the **App-Netbackup-NRPE-custom** host template is used.
...
| Service Alias | Service Template                     | Service Description              |
|:--------------|:-------------------------------------|:---------------------------------|
| Tape-Usage    | App-Netbackup-Tape-Usage-NRPE-custom | Check tapes available in library |
...
```bash
dnf install centreon-pack-applications-netbackup-nrpe
```
...
```bash
apt install centreon-pack-applications-netbackup-nrpe
```
...
2. Whatever the license type (*online* or *offline*), install the **Symantec Netbackup** connector through
...
3. Apply the **App-Netbackup-NRPE-custom** template to the host. A list of macros appears. Macros allow you to define how the connector will connect to the resource, and to customize the connector's behavior.
...
| Macro          | Description                                                                                                                                                    | Default value            | Mandatory   |
|:---------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------|:-------------------------|:-----------:|
| CRITICALSTATUS | Define the conditions to match for the status to be CRITICAL (default: '%\{status\} !~ /up/i'). You can use the following variables: %\{display\}, %\{status\} | not %\{status\} =~ /up/i |             |
...
| OKSTATUS         | Define the conditions to match for the status to be OK (default: '%\{status\} == 0') You can use the following variables: %\{display\}, %\{status\}                                                                                            | %\{status\} == 0                              |             |
...
| CRITICALFROZEN   | Set critical threshold for frozen jobs (default: '%\{state\} =~ /active\|queue/ && %\{kb\} == 0'). You can use the following variables:  %\{display\}, %\{status\}, %\{elapsed\}, %\{type\}, %\{kb\}, %\{parentid\}, %\{schedule\}, %\{jobid\} | %\{state\} =~ /active\|queue/ && %\{kb\} == 0 |             |
| WARNINGFROZEN    | Set warning threshold for frozen jobs (default: none) You can use the following variables: %\{display\}, %\{status\}, %\{elapsed\}, %\{type\}, %\{kb\}, %\{parentid\}, %\{schedule\}, %\{jobid\}                                               |                                               |             |
| WARNINGLONG      | Set warning threshold for long jobs (default: none) You can use the following variables: %\{display\}, %\{status\}, %\{elapsed\}, %\{type\}                                                                                                    |                                               |             |
| CRITICALLONG     | Set critical threshold for long jobs (default: none). You can use the following variables: %\{display\}, %\{status\}, %\{elapsed\}, %\{type\}                                                                                                  |                                               |             |
| WARNINGSTATUS    | Define the conditions to match for the status to be WARNING (default: '%\{status\} == 1') You can use the following variables: %\{display\}, %\{status\}, %\{type\}                                                                            | %\{status\} == 1                              |             |
| CRITICALSTATUS   | Define the conditions to match for the status to be CRITICAL (default: '%\{status\} \> 1'). You can use the following variables: %\{display\}, %\{status\}, %\{type\}                                                                          | %\{status\} \> 1                              |             |
| EXTRAOPTIONS     | Any extra option you may want to add to the command (a --verbose flag for example). All options are listed [here](#available-options).                                                                                                                                             | --verbose                                     |             |
...
| Macro         | Description                                                                                        | Default value     | Mandatory   |
|:--------------|:---------------------------------------------------------------------------------------------------|:------------------|:-----------:|
| FILTERSCRATCH | Filter tape scratch (default: 'scratch')                                                           | scratch           |             |
...
```bash
/usr/lib64/nagios/plugins//check\_centreon\_nrpe -H 10.0.0.1 -p 5666 -t 55  -c check_centreon_plugins -a 'apps::backup::netbackup::local::plugin' 'drive-status'  '  \
	--warning-status="" \
...
|:-----------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------|
| dedup-status [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/backup/netbackup/local/mode/dedupstatus.pm)]     | App-Netbackup-Dedup-Status-NRPE-custom   |
| drive-cleaning [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/backup/netbackup/local/mode/drivecleaning.pm)] | App-Netbackup-Drive-Cleaning-NRPE-custom |
| drive-status [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/backup/netbackup/local/mode/drivestatus.pm)]     | App-Netbackup-Drive-Status-NRPE-custom   |
| job-status [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/backup/netbackup/local/mode/jobstatus.pm)]         | App-Netbackup-Job-Status-NRPE-custom     |
...
| tape-usage [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/backup/netbackup/local/mode/tapeusage.pm)]         | App-Netbackup-Tape-Usage-NRPE-custom     |
