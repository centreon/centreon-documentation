...
| SSHPASSWORD     | Define the password associated with the user name. Cannot be used with the sshcli backend. Warning: using a password is not recommended. Use a private key instead located in `/var/lib/centreon-engine/.ssh/` or use `--ssh-priv-key` if it's stored elsewhere  |                   |             |
