...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Linux SNMP (Rust)** 
...
Le connecteur de supervision **Linux SNMP (Rust)** apporte un modèle d'hôte :

* **OS-Linux-SNMP-Rs-custom**

Le connecteur apporte les modèles de service suivants
(classés selon le modèle d'hôte auquel ils sont rattachés) :

<Tabs groupId="sync">
<TabItem value="OS-Linux-SNMP-Rs-custom" label="OS-Linux-SNMP-Rs-custom">
...
```bash
dnf install centreon-pack-operatingsystems-linux-snmp-rs
```
...
2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Linux SNMP (Rust)**
...
3. Appliquez le modèle d'hôte **OS-Linux-SNMP-Rs-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
/usr/lib/centreon/plugins/centreon-plugin-rust-snmp -H '10.0.0.1' -c 'my-snmp-community' -v '2c'  \
	--json '/usr/lib/centreon/plugins/rs-collections/operatingsystems-linux-snmp/uptime.json' \
	--warning-seconds='' \
	--critical-seconds='' 
```
...
| Option                | Description                                               |
|:----------------------|:----------------------------------------------------------|
| --warning-free-bytes  | Warning threshold for the metric 'memory.free.bytes'.     |
| --critical-free-bytes | Critical threshold for the metric 'memory.free.bytes'.    |
...
| Option                | Description                                             |
|:----------------------|:--------------------------------------------------------|
| --warning-free-bytes  | Warning threshold for the metric 'swap.free.bytes'.     |
| --critical-free-bytes | Critical threshold for the metric 'swap.free.bytes'.    |
| --warning-bytes       | Warning threshold for the metric 'swap.usage.bytes'.    |
| --critical-bytes      | Critical threshold for the metric 'swap.usage.bytes'.   |
| --warning-prct        | Warning threshold for the metric 'swap.usage.percent'.  |
| --critical-prct       | Critical threshold for the metric 'swap.usage.percent'. |
