...
Vous aurez besoin d'un accès au serveur de base de données Microsoft Dynamics AX avec un nom de serveur valide. Dans certains cas, des informations d'identification telles qu'un nom d'utilisateur et un mot de passe peuvent également être nécessaires en fonction de la configuration de sécurité en place.
...
3. Appliquez le modèle d'hôte **App-Dynamics-AX-Database-Mssql-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
