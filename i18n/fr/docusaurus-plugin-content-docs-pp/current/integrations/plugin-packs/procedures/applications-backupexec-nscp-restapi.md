...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Veritas Backup Exec NSCP API** 
...
Le connecteur de supervision **Veritas Backup Exec NSCP API** apporte un modèle d'hôte :
...
|:--------------------------------------|:-------------------------------------------------------------------------------------------------------|
| App-Backupexec-Nscp-Restapi-Disk-Name | Utilise le nom du disque pour découvrir les partitions du disque et supervise l'occupation de l'espace |
...
2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Veritas Backup Exec NSCP API**
...
3. Appliquez le modèle d'hôte **App-Backupexec-Nscp-Restapi-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
| list-disks [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/backup/backupexec/local/mode/listdisks.pm)] | Utilisé pour la découverte de services    |
