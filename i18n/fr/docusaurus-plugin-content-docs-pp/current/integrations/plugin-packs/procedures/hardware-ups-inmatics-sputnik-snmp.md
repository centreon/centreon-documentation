...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Inmatics PSU Sputnik SNMP** 
...
Le connecteur de supervision **Inmatics PSU Sputnik SNMP** apporte un modèle d'hôte :
...
2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Inmatics PSU Sputnik SNMP**
...
```bash
dnf install centreon-plugin-Hardware-Ups-Inmatics-Sputnik-Snmp
```
...
```bash
apt install centreon-plugin-hardware-ups-inmatics-sputnik-snmp
```
...
```bash
yum install centreon-plugin-Hardware-Ups-Inmatics-Sputnik-Snmp
```
