...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Zyxel SNMP** 
...
<Tabs groupId="sync">
<TabItem value="Net-Zyxel-SNMP-custom" label="Net-Zyxel-SNMP-custom">

| Alias    | Modèle de service              | Description                                         |
|:---------|:-------------------------------|:----------------------------------------------------|
| Cpu      | Net-Zyxel-Cpu-SNMP-custom      | Contrôle du taux d'utilisation du CPU de la machine |
| Memory   | Net-Zyxel-Memory-SNMP-custom   | Contrôle du taux d'utilisation des mémoires         |
| Sessions | Net-Zyxel-Sessions-SNMP-custom | Contrôle le nombre de sessions                      |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **Net-Zyxel-SNMP-custom** est utilisé.
...
|:----------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SNMP Agents     | Découvre les ressources via un scan réseau SNMP. Installez le connecteur [Generic SNMP](./applications-protocol-snmp.md) pour obtenir la règle de découverte et créez un modificateur pour le modèle d'hôte **Net-Zyxel-SNMP-custom**. |
...
| Nom de la règle             | Description                                                                                       |
|:----------------------------|:--------------------------------------------------------------------------------------------------|
| Net-Zyxel-SNMP-Traffic-Name | Découvre les interfaces réseau en utilisant leur nom et supervise leur statut et leur utilisation |
...
```bash
dnf install centreon-pack-network-zyxel-snmp
```
...
```bash
apt install centreon-pack-network-zyxel-snmp
```
...
```bash
yum install centreon-pack-network-zyxel-snmp
```
...
```bash
dnf install centreon-plugin-Network-Zyxel-Snmp
```
...
```bash
apt install centreon-plugin-network-zyxel-snmp
```
...
3. Appliquez le modèle d'hôte **Net-Zyxel-SNMP-custom**.
...
| Macro              | Description                                                                                                                                      | Valeur par défaut | Obligatoire |
|:-------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------|:------------------|:-----------:|
| WARNINGFLASHUSAGE  | Threshold                                                                                                                                        |                   |             |
| CRITICALFLASHUSAGE | Threshold                                                                                                                                        |                   |             |
| WARNINGMEMUSAGE    | Threshold                                                                                                                                        |                   |             |
| CRITICALMEMUSAGE   | Threshold                                                                                                                                        |                   |             |
| EXTRAOPTIONS       | Any extra option you may want to add to the command (a --verbose flag for example). Toutes les options sont listées [ici](#options-disponibles). |                   |             |
...
| CRITICALSTATUS     | Define the conditions to match for the status to be CRITICAL. You can use the following variables: %\{activestatus\}, %\{connectstatus\}, %\{display\} | %\{connectstatus\} eq "disconnected" |             |
| WARNINGSTATUS      | Define the conditions to match for the status to be WARNING. You can use the following variables: %\{activestatus\}, %\{connectstatus\}, %\{display\}  |                                      |             |
...
/usr/lib/centreon/plugins/centreon_zyxel_snmp.pl \
	...
	--filter-name='' \
	--warning-status='' \
	--critical-status='%\{connectstatus\} eq "disconnected"' \
	--warning-traffic-in='' \
	--critical-traffic-in='' \
	--warning-traffic-out='' \
...
```bash
OK: All VPN tunnels are OK | 'vpn1#traffic-in'=92073b/s;;;0; 'vpn2#traffic-in'=38359b/s;;;0; 'vpn1#traffic-out'=24045b/s;;;0; 'vpn2#traffic-out'=28064b/s;;;0; 
```
...
|:-----------------------------------------------------------------------------------------------------------------------------|:-------------------------------------------------------------------------------------|
| cpu [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/network/zyxel/snmp/mode/cpu.pm)]                   | Net-Zyxel-Cpu-SNMP-custom                                                            |
| interfaces [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/snmp_standard/mode/interfaces.pm)]          | Net-Zyxel-Traffic-Generic-Name-SNMP-custom<br />Net-Zyxel-Traffic-Global-SNMP-custom |
...
| list-vpn [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/network/zyxel/snmp/mode/listvpn.pm)]          | Non utilisé dans ce connecteur de supervision                                        |
| memory [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/network/zyxel/snmp/mode/memory.pm)]             | Net-Zyxel-Memory-SNMP-custom                                                         |
| sessions [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/network/zyxel/snmp/mode/sessions.pm)]         | Net-Zyxel-Sessions-SNMP-custom                                                       |
| vpn-status [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/network/zyxel/snmp/mode/vpnstatus.pm)]      | Net-Zyxel-Vpn-Status-SNMP-custom                                                     |
...
| Option       | Description                                                     |
|:-------------|:----------------------------------------------------------------|
| --warning-*  | Warning threshold. Can be: 'mem-usage' (%), 'flash-usage' (%).  |
| --critical-* | Critical threshold. Can be: 'mem-usage' (%), 'flash-usage' (%). |
...
| --warning-status  | Define the conditions to match for the status to be WARNING. You can use the following variables: %\{activestatus\}, %\{connectstatus\}, %\{display\}                                                    |
| --critical-status | Define the conditions to match for the status to be CRITICAL (default: '%\{connectstatus\} eq "disconnected"'). You can use the following variables: %\{activestatus\}, %\{connectstatus\}, %\{display\} |
