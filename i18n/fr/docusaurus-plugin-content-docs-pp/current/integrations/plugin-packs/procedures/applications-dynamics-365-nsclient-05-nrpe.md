...
Le connecteur de supervision **Microsoft Dynamics365 NSClient NRPE** apporte un modèle d'hôte :

* **App-Dynamics-365-NRPE-custom**

Le connecteur apporte les modèles de service suivants
(classés selon le modèle d'hôte auquel ils sont rattachés) :

<Tabs groupId="sync">
<TabItem value="App-Dynamics-365-NRPE-custom" label="App-Dynamics-365-NRPE-custom">

| Alias      | Modèle de service                       | Description                                                                    |
|:-----------|:----------------------------------------|:-------------------------------------------------------------------------------|
| New-Orders | App-Dynamics-365-New-Orders-NRPE-custom | Contrôle permettant de vérifier la présence de nouvelles commandes et leur âge |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **App-Dynamics-365-NRPE-custom** est utilisé.
...
Pour superviser les ressources *Microsoft Dynamics365* via NRPE, installez la version Centreon de l'agent NSClient++.
...
```bash
dnf install centreon-pack-applications-dynamics-365-nsclient-05-nrpe
```
...
```bash
apt install centreon-pack-applications-dynamics-365-nsclient-05-nrpe
```
...
```bash
yum install centreon-pack-applications-dynamics-365-nsclient-05-nrpe
```
...
| Macro        | Description                                                                                                                              | Valeur par défaut                                                | Obligatoire |
|:-------------|:-----------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------|:-----------:|
| PATHS        | The path to search for files under                                                                                                       | C:/D365/Interfaces/Input/SalesOrders/New/                        |             |
| PATTERN      | The pattern of files to search for (works like a filter but is faster and can be combined with a filter)                                 | *.xml                                                            |             |
...
| WARNING      | Filter which marks items which generates a warning state.                                                                                | count > 20                                                       |             |
| CRITICAL     | Filter which marks items which generates a critical state.                                                                               | age > -1d or count > 100                                         |             |
...
| query [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/apps/nsclient/restapi/mode/query.pm)]                   | App-Dynamics-365-NRPE-custom          |
