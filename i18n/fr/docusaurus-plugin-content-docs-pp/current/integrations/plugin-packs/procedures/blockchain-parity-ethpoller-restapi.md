...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Ethpoller API**
...
Le connecteur de supervision **Ethpoller API** apporte un modèle d'hôte :
...
2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Ethpoller API**
...
3. Appliquez le modèle d'hôte **Blockchain-Parity-Ethpoller-Restapi-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
| ETHPOLLEREXTRAOPTIONS | Any extra option you may want to add to every command (a --verbose flag for example). Toutes les options sont listées [ici](#options-disponibles). |                   |             |
