...
Le connecteur de supervision **EMC Celerra** apporte un modèle d'hôte :

* **HW-Storage-EMC-Celerra-SSH-custom**
...
|:----------|:--------------------------------------------|:--------------------------------------------------------------|
| Getreason | HW-Storage-EMC-Celerra-Getreason-SSH-custom | Contrôle le statut des 'control station' et des 'data movers' |

> Les services listés ci-dessus sont créés automatiquement lorsque le modèle d'hôte **HW-Storage-EMC-Celerra-SSH-custom** est utilisé.
...
```bash
dnf install centreon-pack-hardware-storage-emc-celerra-ssh
```
...
```bash
apt install centreon-pack-hardware-storage-emc-celerra-ssh
```
...
```bash
yum install centreon-pack-hardware-storage-emc-celerra-ssh
```
...
```bash
dnf install centreon-plugin-Hardware-Storage-Emc-Celerra-Ssh
```
...
```bash
apt install centreon-plugin-hardware-storage-emc-celerra-ssh
```
...
3. Appliquez le modèle d'hôte **HW-Storage-EMC-Celerra-SSH-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
```bash
/usr/lib/centreon/plugins/centreon_emc_celerra_local.pl \
	--plugin=storage::emc::celerra::local::plugin \
	--mode=getreason \
	--hostname='10.0.0.1' \
	--ssh-backend='libssh' \
	--ssh-username='' \
...
```bash
OK: Control station 'primary' status is 'Primary Control Station' |
```
...
|:----------------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------|
| filesystems [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/storage/emc/celerra/local/mode/filesystems.pm)] | Not used in this Monitoring Connector       |
| getreason [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/storage/emc/celerra/local/mode/getreason.pm)]     | HW-Storage-EMC-Celerra-Getreason-SSH-custom |
| pools [[code](https://github.com/centreon/centreon-plugins/blob/develop/src/storage/emc/celerra/local/mode/pools.pm)]             | Not used in this Monitoring Connector       |
...
| --filter                                   |   Exclude the items given as a comma-separated list (example: --filter=datamover). You can also exclude items from specific instances: --filter=datamover,slot\_2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| --threshold-overload                       |   Use this option to override the status returned by the plugin when the status label matches a regular expression (syntax: section,\[instance,\]status,regexp). Example: --threshold-overload='datamover,CRITICAL,^(?!(normal)$)'                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
