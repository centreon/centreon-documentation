...
Le connecteur de supervision **Barco ClickShare Rest API** apporte un modèle d'hôte :
...
3. Appliquez le modèle d'hôte **HW-Device-Barco-Cs-Restapi-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
...
| BARCOCSAPIEXTRAOPTIONS | Any extra option you may want to add to every command (a --verbose flag for example). Toutes les options sont listées [ici](#options-disponibles). |                   |             |
