...
Les connecteurs de supervision suivants sont automatiquement installés lors de l'installation du connecteur **Hyperledger API**
...
Le connecteur de supervision **Hyperledger API** apporte un modèle d'hôte :
...
L’exporter Hyperledger doit être installé, accessible depuis le collecteur Centreon via HTTP ou HTTPS, et exposer une API disponible sur une URL et un port accessibles depuis le collecteur.
...
2. Quel que soit le type de la licence (*online* ou *offline*), installez le connecteur **Hyperledger API**
...
3. Appliquez le modèle d'hôte **Blockchain-Hyperledger-Exporter-custom**. Une liste de macros apparaît. Les macros vous permettent de définir comment le connecteur se connectera à la ressource, ainsi que de personnaliser le comportement du connecteur.
