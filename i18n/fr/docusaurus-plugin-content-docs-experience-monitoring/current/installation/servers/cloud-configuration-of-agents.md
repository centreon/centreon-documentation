...
- **[Token d'accès](../tokens.md)** : le jeton d’identification doit être identique pour tous les agents Experience Monitoring appartenant à la même licence et au même site. Il est stocké dans le fichier de configuration `/etc/quanta/agent.yml`. Le jeton indique à l’agent à quel site appartiennent les données supervisées.
