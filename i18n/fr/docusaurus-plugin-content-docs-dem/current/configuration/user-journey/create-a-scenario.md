...
DEM génère un nouveau parcours avec une seule étape: la navigation vers la racine de votre nom de domaine.
...
Par défaut, DEM cliquera sur la première occurence détectée. Vous pouvez choisir de:
...
Par défaut, DEM envoie le formulaire une fois rempli. Mais vous pouvez modifier ce comportement. Vos options sont:
...
Choisissez un nom pour désigner ce parcours dans les rapports et dans les différents écrans de DEM.
...
Par défaut, DEM considère que le parcours est en échec en cas de problème de sécurité de ce type. Désactivez l’option pour ignorer ces erreurs.
