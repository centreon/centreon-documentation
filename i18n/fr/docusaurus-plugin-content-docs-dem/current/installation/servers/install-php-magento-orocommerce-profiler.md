...
Le module PHP CXM se présente sous la forme d'une extension PHP et vous permettra de remonter des informations concernant le temps d'exécution de chaque partie de votre CMS.
...
| Distribution | Version OS | Version PHP | Module PHP CXM |
|--------------|------------|-------------|-------------------|
...
### Installer le module PHP CXM pour Debian / Ubuntu

Il vous faudra avoir ajouté les dépôts CXM, vous avez déjà dû le faire lors de l'installation de l'agent.
...
### Installer le module PHP CXM pour CentOS / RedHat
...
Il est nécessaire de modifier la configuration du module afin que les évènements de type "Magento" remontent correctement dans CXM.
...
Lorsque tout est installé, **et que le scénario de navigation web est créé dans CXM**, allez dans le menu Scénario Web, puis cliquez sur "+ détails" dans la légende sur l'une des pages nécessitant l'exécution de PHP (exemple : une page Panier qui n'est généralement jamais en cache).
