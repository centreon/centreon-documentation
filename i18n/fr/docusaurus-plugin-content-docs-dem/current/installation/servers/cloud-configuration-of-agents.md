...
- **Token**: Le token d'identification doit être le même pour tous les agents DEM d'une même licence et d'un même site, celui-ci se trouve dans le fichier de configuration "/etc/quanta/agent.yml". Le token indique à l'agent à quel site appartiennent les données monitorées.
- **Hostid**: Celui-ci se trouve également dans le fichier de configuration "/etc/quanta/agent.yml". Le hostid est un identifiant unique permettant à DEM d'identifier de manière unique une instance:
