...
La mise à jour se fait de manière automatique lors de l'installation de l'agent DEM, nous arrêterons de requêter l'agent Zabbix dès lors que nous recevrons des métriques du nouvel agent.

Vous pourrez vérifier que votre serveur remonte bien des données via le nouvel agent en vérifiant la présence du flag "Nouvel agent" dans la configuration de vos serveurs dans DEM.
