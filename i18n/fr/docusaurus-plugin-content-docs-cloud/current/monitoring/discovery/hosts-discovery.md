...
vous devrez choisir manuellement dans la liste des hôtes découverts quels hôtes ajouter à la liste des hôtes supervisés (visible à la page **Configuration > Hôtes > Hôtes (simplifiés)**).
...
- **Analyse automatique** : L'analyse traitera le résultat automatiquement selon l'option sélectionnée (au moins une option doit être cochée) :
    ...
    - **Ajouter les hôtes à la configuration quand ils sont découverts pour la
          ...
          la page **Configuration > Hôtes > Hôtes (simplifiés)**.
...
5. Allez à la page **Configuration > Hôtes > Hôtes (simplifiés)** : les hôtes que vous venez de créer apparaissent dans la liste.
...
2. Depuis ce panneau, modifiez les paramètres de la tâche.
  ...
  Si le module d'autodécouverte découvre un hôte avec un nom déjà existant, ayant été créé manuellement à la page **Configuration > Hôtes > Hôtes (simplifiés)**, cet hôte sera également mis à jour par le module d'autodécouverte, même s'il n'a pas été créé par autodécouverte.
