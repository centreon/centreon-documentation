...
Après avoir lancé ces commandes sur les deux nœuds, copier le contenu du fichier et le coller dans (à créer) `~/.ssh/authorized_keys` sur l'autre noeud puis appliquer les bons droits sur le fichier (toujours en tant que `mysql`) :
...
Lors de la migration de centreon-ha depuis une plateforme Centreon existante ou un déploiement de VM par OVA/OVF, mettez à jour le mot de passe `'@MARIADB_CENTREON_USER@'@'localhost'`:
...
Pour s'assurer que les dernières étapes ont été effectuées correctement, et que les bons noms, logins et mots de passes ont bien été saisis dans le fichier de configuration, il faut lancer la commande depuis les noeuds database (ou depuis n'importe quel noeud si vous avez ajouté les comptes optionnels) :
