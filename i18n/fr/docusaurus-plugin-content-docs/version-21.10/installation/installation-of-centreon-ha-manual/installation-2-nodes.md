...
Pour sécuriser le bon fonctionnement du cluster même en cas de panne du service de résolution de noms, il est impératif que les nœuds se connaissent *via* le fichier `/etc/hosts`. Dans le cas d'un cluster à 2 noeuds il n'est pas nécessaire de déclarer les hôtes `@DB_MASTER_NAME@` et `@DB_MASTER_SLAVE@` puisque ce sont les mêmes que les centraux
...
L'échange de clés doit ensuite être validé par une première connexion qui permettra d'accepter la signature du serveur SSH en tapant "yes" (toujours en tant que `mysql`) :
