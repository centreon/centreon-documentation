...
| --------- | ------------------------- |
| La reconstruction des fichiers RRD d'un service rend invalide le statut du fichier RRD. Toutes ses données historiques sont affichées comme CRITIQUE. | Voici une solution de contournement pour procéder correctement à la reconstruction (sur le serveur central) : <ol><li>Faites une sauvegarde de tous les fichiers de statut RRD liés aux services que vous souhaitez reconstruire.</li><li>Lancez la reconstruction RRD.</li><li>Une fois la reconstruction terminée, restaurez les fichiers RRD (en utilisant la sauvegarde).</li></ol>Si la reconstruction a déjà été effectuée, vous ne pourrez pas récupérer les données antérieures. Supprimez le fichier de statut RRD et toutes les nouvelles données seront affichées correctement avec le bon statut. |
