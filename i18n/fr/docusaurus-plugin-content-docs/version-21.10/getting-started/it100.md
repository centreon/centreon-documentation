...
- Vous aurez accès à la fonctionnalité de [découverte automatique des hôtes et services](../monitoring/discovery/introduction.md), et à la totalité de la bibliothèque de [plugin packs Centreon](../integrations/plugin-packs/introduction.md).
...
* Vérifiez que l'email que vous avez entré dans le formulaire d'inscription IT 100 est correct.
...
* Vérifiez si une licence est bien enregistrée sur le serveur. Si c'est le cas, le fichier suivant existe bien : **/etc/centreon/license.d/epp.license**.

* Vérifiez si l'empreinte du serveur est correcte.

  Votre licence est liée à l'empreinte de votre serveur. Dans certains cas, cette empreinte peut changer (par exemple en cas de modification des interfaces réseau).
  Récupérez l'empreinte du serveur (à la page **Administration > Extensions > Gestionnaire**, cliquez sur **Récupérer l'empreinte**) et comparez-la avec l'empreinte enregistrée dans le fichier **/etc/centreon/license.d/epp.license**. Si les deux empreintes ne correspondent pas :
    1. Demandez un nouveau jeton basé sur la bonne empreinte.
    2. À la page **Administration > Extensions > Gestionnaire**, cliquez sur **Voir la licence**, puis sur **Unlink license**.
    3. Cliquez sur **Télécharger une licence** pour enregistrer la nouvelle licence.
...
* Le log d'erreur suivant :
  * **Centreon 21.10**: /var/opt/rh/rh-php74/log/php-fpm/centreon-error.log
