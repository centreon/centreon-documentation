...
- Fixed interpretation of HTML carriage return.
- Fixed an issue causing MAP server to crash at start up due to service group loading.
- [Maps list] Fixed issue sometimes preventing maps list from being displayed.
- [Viewer] Fixed issue that prevented widget values from being updated in real time.
- [Viewer] Fixed an issue that made metric links disappear when resource status is Unknown.
- [Viewer] Fixed an issue that caused status of disabled resources to be used in inherited status computation.
...
- [MBI] Fixed a calculation issue for VMWare-Cluster-Performances with multiple datastores monitored by one service.	
...
- [IT Extension] Added dependency to centreon-license-manager to validate license.
