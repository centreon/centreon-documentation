...
> Accepter toutes les clés GPG proposées et penser a redémarrer votre serveur
...
Passer maintenant à [l'étape suivante](#configuration).
...
Sécuriser votre installation MariaDB en exécutant la commande suivante :
```shell
mysql_secure_installation
```

Créer enfin un utilisateur avec privilèges **root** nécessaire à l'installation de
Centreon :
...
> Remplacer **<IP\>** par l'adresse IP avec laquelle le serveur Centreon
> Remote Server se connectera au serveur de base de données.
>
> Remplacer **<USER\>** et **<PASSWORD\>** par les identifiants de
> l'utilisateur.

Une fois l'installation terminée, supprimer cet utilisateur via la
commande :
...
Puis exécuter la commande suivante (sur le serveur distant si vous utiliser une base de données locale, sinon sur le serveur de base de données déporté):
...
Depuis MariaDB 10.5, il est obligatoire de sécuriser l'accès en root à la base avant d'installer Centreon. Si vous utiliser une base de données locale, exécuter la commande suivante sur le serveur central :
...
* Définir obligatoirement un mot de passe pour l'utilisateur **root** de la base de données.
Ce mot de passe vous sera demandé lors de l'[installation web](../web-and-post-installation.md).

> Pour plus d'informations, consulter la [documentation officielle MariaDB](https://mariadb.com/kb/en/mysql_secure_installation/).

## Étape 4 : Installation web

Avant de démarrer l'installation web, démarrer le serveur Apache avec la
commande suivante :
...
Terminer l'installation en réalisant les
...
Pour transformer le serveur en serveur distant et l'enregistrer sur le serveur Central, exécuter la commande suivante sur le futur serveur distant :
...
> Remplacer **<IP_TARGET_NODE>** par l'adresse IP du serveur Central auquel vous voulez rattacher le serveur distant (adresse IP vue par le serveur distant).
...
> L'URL d'accès n'est pas complète ou invalide. Utiliser l'option **-root** pour définir le chemin de l'URL de l'API.
