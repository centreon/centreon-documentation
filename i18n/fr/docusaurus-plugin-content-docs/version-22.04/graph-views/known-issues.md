...
| Il se peut que vous rencontriez parfois un retour à la ligne illogique pour les mots sur l'interface web (mots coupés en deux). | Ajustez la taille ou l'étiquette de l'élément pour obtenir un rendu parfait sur l'interface web (mais peut-être pas sur le client Desktop). |
| Il se peut que votre client Desktop se déconnecte sans raison apparente. Vous êtes déconnecté et revenez à la page de connexion. Cela peut se produire lors du démarrage du client Desktop ou de la migration de MAP 3 à MAP 4. | Cela peut être lié au temps nécessaire au transfert des données. Ouvrez le fichier Centreon-Map4.ini sur le client Desktop et ajoutez -Dread.timeout=600 à la fin. Redémarrez ensuite le client Desktop pour appliquer les modifications. |
...
| Le message "Le serveur MAP est déconnecté de Broker" peut occasionnellement apparaître sur le client Desktop. | Ignorez ce message. |
...
| La suppression d'images sur le serveur central Centreon n'est pas appliquée instantanément au client Desktop. | Redémarrez le client Desktop pour que l'image ou les images supprimées disparaissent. |
