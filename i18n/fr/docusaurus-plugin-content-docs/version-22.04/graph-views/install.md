...
| *Services surveillés*          | < 10 000                | < 20 000             |  < 40 000            |  > 40 000                    |
| ------------------------------ | ----------------------- | -------------------- | -------------------- | ---------------------------- |
...
Pour ce faire, vous devez contacter le support [équipe de support de Centreon](https://centreon.force.com/) pour obtenir et installer votre clé de licence.
...
Si une mise à jour de ces navigateurs pris en charge devait entraîner une incompatibilité, Centreon travaillerait sur un correctif dans les plus brefs délais (pour les versions Centreon prises en charge).
...
- 4 GB of RAM minimum, 8 GB conseillés (obligatoire pour 10 000 services ou plus)
- **Java 64 bits version 8**
- La résolution doit être d'au moins 1280 x 768.
- Debian 7,8 or 9

> Desktop Client n'est pas compatible avec Microsoft Windows Server. * Si une version de Java autre que 8 est installée, envisagez d'installer Java 8 et de modifier le fichier Centreon-Map4.ini pour ajouter la ligne suivante : `-vm $path_to_java8$` AVANT `-vmwargs`.
...
- le courtier Centreon Central, généralement sur la machine Centreon Central, en utilisant le port TCP 5758
...
Vous devez fournir au serveur Centreon MAP un utilisateur dédié **qui a accès à toutes les ressources** par le biais des [groupes de listes d'accès] appropriés (../administration/access-control-lists.md).
...
Exécute le script de configuration du serveur MAP de Centreon. Deux modes sont disponibles : interactif ou automatique.
...
Allez dans **Administration > Extensions** et cliquez sur le bouton "Installer" du widget. Le résultat après installation :
...
Vous pouvez trouver les installateurs dans **Monitoring > Map > Desktop Client** ou [ici](https://download.centreon.com/?action=product&product=centreon-map&version=22.04&secKey=9ae03a4457fa0ce578379a4e0c8b51f2).

> Pour des raisons de performance, nous recommandons fortement d'avoir moins de 5, 10 utilisateurs maximum connectés en même temps pour manipuler les vues.
