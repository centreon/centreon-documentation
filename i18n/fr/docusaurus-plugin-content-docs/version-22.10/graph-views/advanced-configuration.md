...
## Surveillance de votre serveur Centreon MAP après l'installation
...
| ACTUATORAPIUSERNAME     | Le nom d'utilisateur de l'API   |
| ACTUATORAPIPASSWORD     | Le mot de passe de l'API        |
...
Le script de sauvegarde est exécuté quotidiennement (à 2 heures du matin) par une tâche cron située dans `/etc/cron.d/centreon-map-server-backup` :
...
### Restauration des données du serveur Centreon MAP
...
Modifiez le fichier de configuration studio-config.properties situé dans `/etc/centreon-studio` :
...
Vous devriez voir l'état du serveur :
