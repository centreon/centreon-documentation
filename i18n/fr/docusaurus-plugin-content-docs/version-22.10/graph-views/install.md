...
> Centreon MAP nécessite une clé de licence valide. Pour en acquérir une et récupérer les référentiels nécessaires, contactez [Centreon](mailto:sales@centreon.com).
...
Avant de procéder à l'installation, veillez à consulter le chapitre sur les conditions préalables pour connaître la configuration requise (unité centrale et mémoire).
...
- L'interface Web du Centreon MAP, développée en Javascript et basée sur [Backbone.js](http://backbonejs.org/).
- Client de bureau Centreon MAP, développé en Java, basé sur [Eclipse RCP 4](https://wiki.eclipse.org/Eclipse4/RCP).
...
| Application       | Source            | Destination                          | Port      | Protocole  | Finalité |
|-------------------|-------------------|--------------------------------------|-----------|----------- |-----------------------------------------------------|
| Serveur de cartes | Serveur de cartes | Courtage central Centreon            | 5758      | TCP        | Obtenir des mises à jour de l'état en temps réel |
| Map Server        | Serveur de cartes | Base de données Centreon MariaDB     | 3306      | TCP        | Récupérer la configuration et d'autres données de Centreon |
| Serveur de cartes | Serveur de cartes | Base de données du serveur de cartes | 3306      | TCP        | Stocke toutes les vues et les données relatives à Centreon MAP |
...
| Client de bureau  | Utilisateur       | Serveur de cartes                    | 8080/8443 | HTTP/HTTPS | Récupération et création de vues et de contenu
...
Pour ce faire, vous devez contacter le support [équipe de support Centreon](https://centreon.force.com/) pour obtenir et installer votre clé de licence.
...
| ------------------------- | ----------------------- | -------------------- | -------------------- | ----------------------------------- |
| *CPU*                     | 2 vCPU ( 3Ghz ) minimum | 4 CPU (3GHz) Minimum | 4 CPU (3GHz) Minimum | Demandez à l'assistance de Centreon |
| *Mémoire dédiée*          | 2GB                     | 4GB                  | 8GB                  | Demandez à l'assistance de Centreon |
| *Partition Data MariaDB*  | 2GB                     | 5GB                  | 10GB                 | Demandez à l'assistance de Centreon |
...
Pour ce faire, vous devez contacter le support [Centreon support team](https://centreon.force.com/) pour obtenir et installer votre clé de licence.
...
* Mozilla Firefox (dernière version au moment de la sortie du logiciel Centreon ou d'une version ultérieure).
...
* Apple Safari (dernière version au moment de la sortie du logiciel Centreon ou d'une version plus récente).
* Microsoft Edge Chromium (dernière version au moment de la sortie du logiciel Centreon et au-delà).

Si une mise à jour de ces navigateurs pris en charge devait entraîner une incompatibilité, Centreon travaillerait sur un correctif dans les plus brefs délais (pour les versions Centreon prises en charge). Bien que d'autres navigateurs puissent fonctionner, Centreon ne tentera pas de résoudre les problèmes liés aux navigateurs autres que ceux énumérés ci-dessus.

La résolution de votre écran doit être d'au moins 1280 x 768.

### Client de bureau Centreon MAP

- 4 Go de RAM minimum, 8 Go conseillés (obligatoire pour 10 000 services ou plus)
- **Java 64 bits version 8**
- La résolution doit être d'au moins 1280 x 768.
- Debian 7,8 ou 9

> Desktop Client n'est pas compatible avec Microsoft Windows Server. * Si une version de Java autre que 8 est installée, envisagez d'installer Java 8 et de modifier le fichier Centreon-Map4.ini pour ajouter la ligne suivante :
>
> `-vm $path_to_java8$` AVANT `-vmwargs`.

Pour optimiser le client de bureau, vous devez lui "donner" plus de mémoire que la valeur par défaut. Modifiez le fichier suivant :
...
La machine Centreon MAP Server doit être accessible :

- Le broker Centreon Central, généralement sur la machine Centreon Central, en utilisant le port TCP 5758.
- La base de données Centreon, généralement sur la machine Centreon Central, en utilisant le port TCP 3306.
- La base de données MAP de Centreon, généralement sur l'hôte local, en utilisant le port TCP 3306.
...
Les machines clientes Centreon MAP Desktop doivent y accéder :

- Centreon MAP Server, en utilisant le port HTTP 8080 ou 8443 lorsque HTTPS/TLS est activé.
...
Vous devez fournir au serveur Centreon MAP un utilisateur dédié **qui a accès à toutes les ressources** par le biais des [groupes de listes d'accès](../administration/access-control-lists.md) appropriés.
...
Exclure l'utilisateur de la politique d'expiration du mot de passe sur la page **Administration > Authentification** : son mot de passe n'expirera jamais.
...
Le privilège INSERT ne sera utilisé que pendant le processus d'installation afin de créer une nouvelle sortie de Centreon Broker. Il sera révoqué ultérieurement.
...
Pour installer le référentiel Centreon, exécutez la commande suivante :
...
Installez le référentiel Centreon MAP, vous pouvez le trouver sur le [portail de support](https://support.centreon.com/s/repositories).
...
Exécuter le script de configuration du serveur MAP de Centreon. Deux modes sont disponibles : interactif ou automatique.
...
S'il s'agit de votre première installation, nous vous conseillons d'utiliser le mode standard (interactif) et de choisir **Non** lorsqu'on vous demande le mode d'installation avancé :

```shell
/etc/centreon-studio/configure.sh
```

Si vous venez d'installer Centreon 22.10, sachez que la plate-forme utilise désormais le nouveau protocole BBDO v3. Pour que MAP fonctionne correctement, modifiez le fichier suivant : `/etc/centreon-map/map-config.properties`.
...
> Avant de redémarrer Broker, vous devez exporter la configuration à partir de l'interface Web Centreon.
...
Téléchargez la licence `map.license` donnée par l'équipe de support. Rafraîchissez la page et la bannière doit être verte avec la date de validité de la licence.
...
Allez dans le menu **Administration > Extensions** et cliquez sur le bouton "Installer" du widget. Le résultat après installation :
...
Le client de bureau est actuellement disponible uniquement pour les plateformes **64-bit** Windows, Mac et Linux (Debian et Ubuntu).

Vous pouvez trouver les installateurs dans **Monitoring > Map > Desktop Client** ou [ici](https://download.centreon.com/?action=product&product=centreon-map&version=22.10&secKey=9ae03a4457fa0ce578379a4e0c8b51f2).

> Pour des raisons de performances, nous recommandons fortement d'avoir moins de 5, 10 > utilisateurs maximum connectés en même temps pour manipuler les vues.

### Installation

#### Sur l'ordinateur de l'utilisateur

Le client de bureau nécessite **Java 8**. Vous pouvez télécharger et installer la dernière version de Java depuis [ici](https://java.com/fr/download/manual.jsp).
...
Vous pouvez installer le logiciel à cet emplacement sans droits d'administrateur, mais vous pouvez changer la destination en fichiers de programme si vous avez les droits suffisants.

Utilisez le programme d'installation pour installer correctement le logiciel et l'intégrer dans l'environnement Windows.
Le programme d'installation peut également être utilisé pour le désinstaller à partir de la page de configuration dédiée de Windows.
...
Une fois installé, le Desktop Client est automatiquement maintenu à jour grâce à un système de mise à jour en ligne.
