...
1. Sur votre plateforme 18.10, remplacez `https://packages.centreon.com/rpm-standard` ou `https://yum.centreon.com/standard/` par `https://archives.centreon.com/standard/` dans votre configuration YUM (par défaut, `/etc/yum.repos.d/centreon.repo`).

2. Mettez à jour votre Centreon 18.10 jusqu'à la dernière version mineure.
...
2. Désinstallez la version actuelle :
  ...
  > Assurez-vous d'avoir [installé le dépôt officiel de MariaDB](./upgrade-from-18-10.md#installer-le-dépôt-mariadb) avant de poursuivre la procédure.
...
Déployez la configuration du collecteur depuis l'interface web en suivant
[cette procédure](../monitoring/monitoring-servers/deploying-a-configuration.md),
