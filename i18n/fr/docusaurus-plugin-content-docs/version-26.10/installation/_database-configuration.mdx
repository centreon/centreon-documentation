...
Définissez un mot de passe pour l'utilisateur root de la base de données en exécutant la commande suivante :
...
le processus d'installation web (à l'[étape 6](./web-and-post-installation.md#étape-6--database-information),
...
Veuillez noter que la configuration suivante n'est qu'un exemple, pour une infrastructure disposant de 8 Go de RAM. Il appartient à chaque utilisateur d'adapter la configuration de la base de données à ses besoins.
Nous vous invitons à vous rapprocher de votre administrateur de base de données pour obtenir une configuration qui réponde à vos besoins.
...
> ```
>
> Vous pouvez également ajuster l'utilisation de la mémoire et des tampons des tables gérées par le moteur
> InnoDB. L'exemple ci-dessous s'applique à un serveur de base de données disposant de 8 Go de RAM.
>
> ```shell
