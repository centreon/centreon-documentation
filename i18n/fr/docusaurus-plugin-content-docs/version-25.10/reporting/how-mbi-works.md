...
Chaque jour, MBI travaille en exécutant 3 phases principales indépendantes :

* [Le serveur central prépare les données brutes](#phase-1--les-données-sont-préparées-par-le-serveur-central).
* [L'ETL copie les données du jour précédent sur le serveur MBI et les agrège](#phase-2-the-etl-is-launched-data-is-copied-to-mbi-and-aggregated). Les données sont alors prêtes à être utilisées dans les rapports.
* Aux heures prévues, [CBIS recueille les données pertinentes pour un rapport et le génère](#phase-3-cbis-generates-the-reports).
...
2. Sur le serveur central, un script (**eventReportBuilder**) est lancé par un cronjob tous les jours à 3h du matin : il convertit les résultats des vérifications en [événements](https://docs.centreon.com/fr/docs/reporting/concepts/#event) qui seront utilisés pour calculer la disponibilité. L'heure à laquelle ce script est lancé est définie dans **/etc/cron.d/centreon**.

> La conversion des résultats des vérifications en événements doit être totalement terminée **avant** [le lancement de l'ETL](#phase-2--lancement-de-letl-les-données-sont-copiées-sur-mbi-puis-agrégées), sinon les rapports seront vides. En cas de doute, vérifiez ce journal pour voir si la conversion est terminée : **/var/log/centreon/eventReportBuilder.log**. 
...
2. **/usr/share/centreon-bi/etl/dimensionsBuilder.pl** : MBI prépare une liste de toutes les [dimensions](https://docs.centreon.com/fr/docs/reporting/concepts/#dimension) présentes dans les données importées, pour chaque hôte, service et métrique.

3. **/usr/share/centreon-bi/etl/eventStatisticsBuilder.pl** : MBI calcule la [disponibilité](https://docs.centreon.com/fr/docs/reporting/concepts/#disponibilité) des ressources en fonction des données copiées depuis le serveur central et des dimensions calculées juste avant. La disponibilité de chaque ressource est calculée par jour et par mois, en tenant compte des [périodes temporelles](https://docs.centreon.com/fr/docs/monitoring/basic-objects/timeperiods/) sélectionnées dans le champ **Sélectionner les plages de services pour le calcul des statistiques de disponibilité** de la page **General options**, dans l'onglet **Options de l'ETL**.

4. **/usr/share/centreon-bi/etl/perfdataStatisticsBuilder.pl** : MBI agrège toutes les métriques par heure/jour/mois, en tenant compte des [périodes] (https://docs.centreon.com/fr/docs/monitoring/basic-objects/timeperiods/) sélectionnées dans le champ **Sélectionner les plages de services pour le calcul des statistiques de performance** de la page **General options**, dans l'onglet **Options de l'ETL**. Le script inclut le calcul des centiles, s'il est configuré.
...
Notez que pour [BAM](https://docs.centreon.com/fr/docs/service-mapping/introduction/), les données agrégées ont déjà été calculées par le serveur central. L'ETL se contente de les copier sur le serveur MBI.
...
* La périmètre des données dans le rapport est déterminée par l'onglet **Paramètres du rapport** de la tâche. N'oubliez pas que le rapport et toutes ses données seront partagés avec d'autres utilisateurs en fonction de ce que vous avez défini dans les [groupes de tâches](https://docs.centreon.com/docs/reporting/concepts/#job-groups), indépendamment des ACL des autres utilisateurs sur les ressources.
* Les rapports générés sont copiés sur le serveur central, dans **/var/lib/centreon/centreon-bi-server/archives**. (Cette opération est effectuée en exécutant la [règle de publication](reports-publication-rule.md) SFTP globale appelée **Default**.)
* Toutes les autres [règles de publication](reports-publication-rule.md) **globales** sont ensuite exécutées.

## Phase 4 (facultatif) : Les rapports sont envoyés aux utilisateurs
...
Exemple : Centreon peut envoyer des rapports par e-mail selon les paramètres définis dans une [une règle de publication] SMTP(reports-publication-rule.md) ([postfix](https://docs.centreon.com/fr/docs/administration/postfix/) doit être configuré).
