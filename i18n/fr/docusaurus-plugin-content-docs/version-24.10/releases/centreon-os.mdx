...
- [Administration] Fixed an error message when a CMA token was copied.
- [API v1] Fixed creation of services by CLAPI with a non-administrator user.
- [API v1] Forbid creation of host and host groups with an empty name.
...
- [CLAPI] Fixed host creation form to accept spaces in name.
- [Configuration] Fixed a circular dependencies issue for service template inheritance (UI + API).
- [Configuration] Fixed an issue on Additional Configuration where configuration were lost if name are just numbers.
...
- [Dashboards] Metrics Graph widget - Fixed display issue when changing graph display type.
- [Dashboards] Fixed contact search when sharing a dashboard or a playlist.
- [Dashboards] Fixed saving issue when leaving dashboards to a legacy page.
...
- [Resource Status] Fixed an issue on notification tab where same user was displayed multiple times.
- [Resource Status] Fixed notification tab to display the contact set on service template rather than on the host.
- [Resource Status] Removed deleted resources from search results.
