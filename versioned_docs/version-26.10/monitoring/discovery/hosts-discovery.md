...
description: "Create and manage host discovery jobs using wizards, mappers, and policies"
---
...
2. Install the Monitoring Connector for the resources you want to discover.
    ...
    > catalog.
    >
    > If you have set **Automatic installation of plugins** to **ON** on the **Configuration > Connectors > Monitoring Connectors** page, the discovery plugin will be installed automatically when you run the discovery job.
...
- **Manual analysis**: Once the discovery job is executed, you will have to choose manually which hosts should be added to the list of monitored hosts (on the **Configuration > Hosts > Hosts** page). See [Analyze the results of a discovery job](#analyze-the-results-of-a-discovery-job).
...
2. In this panel, edit the job settings.
  ...
  If the autodiscovery module discovers hosts whose names are already used by hosts that have previously been created manually on the **Configuration > Hosts > Hosts** page or with the API or CLAPI, those existing hosts will also be updated by the autodiscovery module.
