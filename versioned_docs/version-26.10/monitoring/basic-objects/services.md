...
* The **Freshness Threshold** is expressed in seconds. If, during this period, no passive command is received from the host, the active check command is executed. A default threshold value is assigned automatically if this field is left blank and the check is enabled.
