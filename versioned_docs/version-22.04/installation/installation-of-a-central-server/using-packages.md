...
> **Note that this deactivation should be temporary.** To enable SELinux, use the same procedure running the following command:
> ```shell
> sed -i s/^SELINUX=.*$/SELINUX=enabled/ /etc/selinux/config
> ```
