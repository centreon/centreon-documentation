...
- [API] Added dashboard shares to dashboards list.
...
- [Authentication] Fixed hardcoded email value that block SSO authentication with OpenId Connect when parameter is not email.
...
- [Install] Fixed installation when database user already exists.
- [Installation] Fixed issue adding twice poller ID in Centreon gorgone configuration.
- [My Account] Fixed account properties edition for local user only.
- [Resource Status] Fixed bad host redirection in the services panel.
- [Resource Status] Fixed default sorting order from worse to best resources status.
- [Web] Fixed an issue while showing herited contacts/contact groups in notification tab.
