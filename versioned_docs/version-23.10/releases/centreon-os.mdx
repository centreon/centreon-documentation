...
- [API v1] Fixed creation of services by CLAPI with a non-administrator user.
- [Configuration] Fixed a circular dependencies issue for service template inheritance (UI + API).
- [Configuration] Fixed host alias not properly handling quotes in input.
...
- [Install] Added monitoring-plugins-basic dependency for installation on Debian platform.
- [Resource Status] Fixed a notification tab issue when using the “Inherit only contacts/contacts group from host” parameter.
