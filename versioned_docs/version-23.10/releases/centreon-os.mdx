...
- [Authentication] When users are authenticated using an IdP, the password fields no longer appear when editing a contact.
...
- [Vulnerability] Fixed XSS in Configuration > Hosts > hostgroups.	
...
- [Custom Views] Host Monitoring - Fixed the export feature for all resources.
- [Install] Added **monitoring-plugins-basic** dependency for installation on Debian platforms.
- [Resource Status] Fixed a notification tab issue that occurred when using the “Inherit only contacts/contacts group from host” parameter.
...
- Securized Poller configuration commands to avoid RCE vulnerability.
...
- [Custom Views] Servicegroup monitoring widget - Fixed issue preventing non admin users from seeing widget data.
- [Metaservices] Fixed SQLi issues. Fixed several issues occuring when using MySQL 8.
...
- [Security] Fixed vulnerability on notification escalation pages.
- [Security] Fixed SQLi vulnerabilities on Configuration > Users > Contact templates.
...
- [Vulnerability] SQL injections are now prevented on Host Category configuration page.
- [Vulnerability] SQL injections are now prevented on the Host dependency field, in the Host configuration page.
...
- [Install] Added missing update file for version 23.10.24.
...
- [Resource Status] The **Select all** checkbox in the view by host now works correctly.
...
- [Media - Image Synchronization] SVG files are now supported.
- [Resource Status] Fixed an issue preventing some services from being listed in the filters.
...
- [Image upload] Fixed a security vulnerability on image upload.
...
- [API Token] Fixed an issue with the expiration date of API-generated tokens.
...
- [Resource status] Fixed issues on tags (filters on host groups, host categories, service groups, service categories).
...
- Fixed an issue on Alma 9 where plugins were not installed because yum did not exist anymore.
