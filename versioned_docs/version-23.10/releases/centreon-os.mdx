...
- [API] Fixed issue when using special characters in metric name.
- [API] Fixed an issue with password clearly displayed in Host & Service API response.
- [Authentication] Fixed match with attribute used to identify users.
- [Authentication] Enabled GUI access option for imported users authenticated through SSO.
- [Core] Fixed access to pages when Symfony debug mode is enabled.
- [Event Logs] Fixed pagination.
- [Packaging] Updated centreontrapd mode on debian central.
- [Widget] Fixed issue when loading widget.
- Fixed crash in partitioning script when database server does not support partitioning.
- Fixed widgets get cloned when creating a new dashboard while editing a dashboard.
...
- Fixed potential XSS vulnerability in listing of discovered hosts.
