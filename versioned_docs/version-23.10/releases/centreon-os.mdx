...
- [API] Added dashboards list ordering possibilities for creator, creation date and update date.
...
- [API] Fixed an issue with categories in POST service and service template.
- [API V1] Fixed an issue on centreon_realtime_hosts, when get/filter by criticality.
- [Resource Status] Fixed a bug occuring when user cleared search bar.
- [Share] Fixed error occurring while granting viewer rights as an administrator.
...
- Fixed a bug on service auto discovery where only half of the host was correctly discovered.
