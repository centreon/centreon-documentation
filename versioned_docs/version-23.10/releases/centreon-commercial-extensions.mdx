...
- [Viewer] Fixed an issue that made breadcrumb not right when selecting a Centreon resource.
- [Viewer] Fixed an issue causing label not to be aligned for resources in geometric mode.
- [Viewer] Fixed an issue that caused image opacity parameter not to be applied.
- [Viewer] Fix container label not being correctly centered in geometric style.
...
- [BAM] The name of a KPI whose type is Business Activity is now correctly displayed in the details of its parent BA.
...
- [Anomaly Detection] Fixed an issue which caused Anomaly Detection not to send real time data to SAAS.
