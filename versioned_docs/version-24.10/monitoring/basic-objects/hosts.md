...
* **Name**: host name used by the Monitoring Engine. Cannot contain `~!$%^&|'"<>?,()=*{}` and spaces will be automatically replaced with _. Using name containing `/` could also create technical impact on SQL request, web UI or other compatibility problems with third-party tools. We recommand only use of `_-.` for hosts name.You may use the `h.name:` syntax in the **Search bar** of the **Resource status** to look up a host.
