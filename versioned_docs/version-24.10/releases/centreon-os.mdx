...
- [Administration] Fixed an issue on user group and template where importing LDAP with non admin user.
- [API Tokens] Fixed an issue in creation form with custom duration.
- [API] Fixed an issue on service template listing endpoint.
- [CLAPI] Fixed an issue where warning was returned when using getmacro.
...
- [Configuration] Fixed an issue where mass Change on Linked Service Templates cause a 500 Internal Server Error.
...
- [Meta] Fixed several issues occuring when using MySQL 8.
- [Resource Status] Default values for monitoring options are now also applied to non-admins.
- [Resource Status] Fixed issue preventing the selection of multiple resources when using advanced filters.
