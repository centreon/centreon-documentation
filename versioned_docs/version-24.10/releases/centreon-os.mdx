...
- [CMA] installer and CLI wording has been harmonized to fit with new UI labels.
...
- [Compatibility] Fixed issue preventing installation from being completed when database tables already exist.
- [Gorgone] Fixed an issue on Alma 9 where plugins were not installed because yum did not exist anymore.
- [Gorgone] Removed useless error present at every first start of gorgone.
