...
- [CLAPI] Fixed an issue where the APPLYTPL command can take a long time to take effect.
...
- [Configuration] Fixed an issue on Agent configuration where results of filters were not as expected if no matching data.
...
- [Configuration] Fixed an issue preventing user from disabling a meta-service.
- [Configuration] Fixed an issue when searching Host in Agent configuration (Poller-initiated connection).
- [Configuration] Fixed an issue where configuration files were not generated when poller was disabled in the listing and re-enabled through the form.
- [configuration] Fixed an issue where configuration wasn't exported for hosts without alias, declared in agent confguration.	
...
- [Downtimes] Fixed issue with meta-service names not being properly displayed.
...
- [LDAP Import] Fixed an issue where LDAP user with a UID containing a special character was imported multiple times.
...
- [Resource Status] Prevented services related to a deleted host to appear in menu filter.
