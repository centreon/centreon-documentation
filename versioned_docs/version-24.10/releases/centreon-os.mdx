...
- [Dashboards] Resource Table widget - Added a link to access OpenTicket on the provider side.
- [Packaging] Increased php memory limit in the centreon php configuration.
- [Performance Graphs] The limit on the number of metrics displayed per chart has been raised to 20. Also fixed an issue with unnecessary legends being displayed when this limit is exceeded.
...
- [Configuration] Fixed an issue when duplicating the configuration of an RRD broker that resulted in missing outputs.
...
- [Performance Graphs] Fixed an issue preventing users from selecting multiple graphs when the paltform's language isn't "en_US".
- [Playlists] Fixed an issue preventing host problems from being displayed.
- [Resource Status] Fixed an issue preventing services from being displayed when filtering by the severity of their parent host.
- [Unattended] Fixed an issue that resulted in incomplete ITEE installations when using the unattended.sh script.
...
- [Security] fixed a vulnerability with the Images parameters.
- [Security] Fixed a vulnerability with LDAP configuration.
- [Security] fixed a vulnerability with the Visual Metrics page.
- [Security] fixed a vulnerability with Hostgroups notifications.	
...
- [Custom Views] Open Ticket - Fixed an issue when filtering on host categories.
...
- [BREAKING CHANGE] [Open Tickets] Due to security concerns, users can no longer define custom commands linked to a rule.
