...
```shell
curl -L https://raw.githubusercontent.com/centreon/centreon/22.10.x/centreon/unattended.sh -O /tmp/unattended
```

3. Run the following command as **root**:

```shell
bash unattended.sh install -t poller -v 22.10 -r stable -l DEBUG  2>&1 |tee -a /tmp/unattended-$(date +"%m-%d-%Y-%H%M%S").log
```
