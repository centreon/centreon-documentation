...
- [Administration] Token expiration date is now displayed with hh:mm.
- [Authentication] SAML IDP form - Added a new field to handle the requested authentication context.
- [CMA] Handle freshness by default for deployed CMA related Monitoring Connector.
- [Configuration] Additional configuration - enhanced password-type fields management.
- [Configuration] Agent configuration now allows any certificate path.
- [Configuration] Agent configuraiton - CMA Token is now mandatory in No TLS.
- [Configuration] Agent configuration - listening port has been added as a parameter in agent-initiated connection.
- [Configuration] Cert format is now accepted for certificates in agent configuration.
...
- [Dashboards] Resource Table widget - Added link to the detail of a ticket on the provider side.
...
- [Administration] Fixed a LDAP issue where user DN was never updated.
- [CLAPI] Fixed an issue where CLAPI returns sometimes a non needed empty return code message.
- [CMA] Handle freshness by default for deployed CMA related Monitoring Connector.
- [Configuration] Added a validation error in tempate.
- [Configuration] Agent configuration now allows any certificate path.
- [Configuration] Cacheert format is now accepted for certificates in agent configuration.
- [Configuration] Cleaned up and fixed macro management in host/HT legacy forms.
...
- [Hosts] Fixed an issue by not allowing to create circular parent/child hosts relationship.
- [Install/upgrade script] is_encryption_ready is now a tinyint instead of an enum.
- [Monitoring] Fixed "hostgroups summary" monitoring page.
- [Performance Graphs] Fixed an issue preventing user from selecting multiple graphs when language selected isn't "en_US".
- [Performance Graphs] Fixed an issue that prevented the filters from working together sequentially (in a funnel).
- [Playlists] Fixed issue preventing host problems from being displayed.
- [Resource Status] Fixed issue preventing services from being displayed when filtering on their parent host severity.
- [Unattended] Fixed an issue in the ITEE installation that could be incomplete when installing a platform with the unattended.sh script.
- [Users] Fixed issue preventing user from being created when using an autologin key.
...
- [Security] Fixed broken object level authorization in Users configuration endpoint (/centreon/api/latest/configuration/users).
- [Security] Fixed an issue where writeEngineSecrets.sh cannot be used with --insecure option.
- [Security] Token changes are logged in a dedicated file with a dedicated logger and channel.
