...
- [CMA] Token is now mandatory in No TLS.
...
- [Broker] Fixed an issue on deleting membership of service to service group.
- [Configuration] Fixed an issue where Engine crashes when notifications are enabled without contact address.
- [Configuration] Fixed an issue where output was truncated for some CMA checks.
- [Configuration] Update logrotate configuration to match packaging permissions.
- [Engine] Fixed an issue where the external command execution can segfault.
- [Engine] Fixed an issue where host severities were not removed from the `resources` table after deletion.
- [Resource status] Fixed an issue where commands were not displayed for CMA Windows services.
...
- [CMA] Added generic native check for custom scripts.
- [CMA] Native checks - Windows - Services - added filter for startup/service type and delayed services.
- [CMA] Token is now mandatory in No TLS.
- [CMA] Windows installer can now handle multiple instances on the same Host.
...
- [CONFIG] Update logrotate configuration to match packaging permissions.
