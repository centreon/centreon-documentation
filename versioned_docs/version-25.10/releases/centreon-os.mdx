...
- CA certificate related columns are created in instances db.
- [Configuration] Provided script to adapt threshold format for CMA native checks.
- [Dashboards] Resource Table widget - Options are now available to enable/disable ticket creation (for hosts and services).
- Dedicated token & agent configuration are created when creating a Poller/Remote Server.
- [Logs] Added Opentelemetry in Engine logs menu.
...
-	[API] Removed requirements of "hosts" field when adding a host..
- [Configuration] Fixed an issue where macros were not exported for service by HG.
- [Configuration] Fixed an issue where the new Host Group configuration page appears twice in the menu.
- [Configuration] Fixed an issue where trailing comma in string leads to sql error for hostgroups during remote server export.
...
- [Dashboards] Fixed issue preventing metric widgets from being displayed.
- [Dashboards] Metrics Graph widget - Resolved an issue preventing meta services from being used in dataset selection.
- [Downtimes] Fixed issue when filtering on services.
- Fixed an issue where Reach API Realtime was always set to "Yes" if User Created With "No" was selected.
- [Host Group] Fixed an issue where the host group configuration name restriction is not aligned with export configuration.
- [LDAP] Fixed an issue where auto import was ignored.
- [Logs] Fixed a PHP warning when running CentreonWorker.
- [Monitoring] Fixed an issue where top counter wrongfully checks password expiration for non-local accounts.
- [Monitoring] Fixed difference of values in graph between the Ressource status and the Performance pages.
...
- [Broker] In case of SC write function does not return a boolean, we delay next write retry.
...
- [Broker][Engine] Fixed an issue where unexpected notifications were sent for UNKNOWN services when host goes DOWN.
...
- [Packaging] Fix gorgoned logrotate configuration file.
...
- [CMA] Full treshold syntax is now handled for all native checks.
...
- [CMA] Fixed uninstaller signature and installer AGENTINSTANCE issues.
