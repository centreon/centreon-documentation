...
- [CMA] Host can now be automaticaly created when option is checked in Agent configuration.
...
- Fixed an issue during remote server upgrade, regarding event_script output for CMA.
- [API] Fixed an issue where reach API realtime was reset to "No" if front-end set to "No".
- [API] When trying to acces the /monitoring/servers endpoint, the API no longer returns "null" (it returns the IP address of the pollers).
...
- [Installation] Fix mariadb password setup.
...
- [CMA] Fixed an issue where peer name was not in peer certificate, while correctly configured in self-signed certificate.
...
- Resolved partitioning and data duplication issues in rebuild process.
...
- Improved wording for CMA installation script output.
...
- [CMA] File native check can now trigger alerts on empty result set.
