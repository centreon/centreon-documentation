...
- [Dashboards] Status Grid widget: new filter on state of resources (unhandled, acknowledged, in downtime, flapping) has been added.
- [Packaging] Increase php memory limit in centreon php configuration.
- [Resource Status] Graphs containing more than 20 metrics are no longer displayed, for performance and readability reasons.
...
- [API] Fixed an issue where using Rest APIv1 changes the token expiration date.
- [API] Fixed commands when requesting services in real time, using MySQL 8.
- [API] Fixed wrong filtering on realtime service categories listing endpoint.
...
- [Configuration] Fixed an issue on Hosts configuration where the host name configuration name restriction not align with export configuration.
...
- [Authentication tokens] Fixed an issue when canceling action in confirmation modal.
- [BA monitoring page] Fixed issue when using dark mode.
- [CEIP] Added data-testid on enable button for both direction.
-	[CLAPI] Fixed an issu where the APPLYTPL command can take a long time to take effect.
- [Core] Fixed an issue with API routes.
- [Configuration] Fixed an error when changing host template order in host form.
...
- [Configuration] Fixed an issue when searching Host in Agent configuration (Poller-initiated connection).
- [Configuration] Fixed an issue where Host category was not displayed in listing when not linked to Host.
- [Configuration] Fixed an issue of page content mix-up when navigating between Host Groups and Additional configuration pages.
- [Configuration] Fixed an issue on bad display of escalation name with special character.
- [Configuration] Fixed an issue on remote servers, where max auto_increment was reached after many exports.
- [Configuration] Fixed an issue when switching CMA agent modes without filling all required fields.
...
- [Dashboards] Fixed an issue preventing to get dashboards filtered by creator by API.
...
- [Dashboards] Single Metric widget - Fixed the real-time retrieval of metric value.
- [Gorgone] Clarified Apache logs when gorgone connexion failed in discovery.
- [Hosts] Fixed an error where ACL Resource Groups field loses its value and displays a number instead of a string when a form error occurs.
- [Licenses] Fixed an error message concerning the creation of a dynamic property in a class when downloading licences.
- [Log] Fixed an issue where Error impCompanyPortal appears on /var/log/centreon/centreon-web.log.
- [Medias] Fixed issue preventing pagination from being displayed properly.
- [Monitoring] Fixed an issue on Summary by host group.
- [Open Tickets] Forced BBDO 3.0.1 instead of 3.1 on pollers, to prevent a blocking issue.
- [Performance Graphs] Fixed issue when exporting data in CSV.
...
- [Vulnerability] Fixed XSS in the ACL Menus Access page.
- [Vulnerability] Fixed XSS in the Hosts Configuration page.
- [Vulnerability] Fixed XSS in the Recurrent Downtimes page.
...
- [CMA] CMA is now taking account retry_interval and max_attempts.
...
- [Packaging] Fixed a issue that cause an upgrade failure due to centreon-perl-libs split into centreon-perl-libs and centreon-perl-libs-common.
...
- [Open Tickets] Fixed issue preventing ticket from being closed when the corresponding resource had its status changed since the opening of the ticket.
- [Translation] Added missing French translations.
