...
|------|-------------|---------|----------------|
| **1. Import event and availability data (excluding performance data)** | Import event data but not performance data (`data_bin`) between specific dates. | <CodeBlock language="shell">{`nohup /usr/share/centreon-bi/etl/importData.pl -r -s $date_start$ -e $date_end$ --ignore-databin --no-purge > /var/log/centreon-bi/rebuild_importDataEvents.log &`}</CodeBlock> | **Fast** (minutes) |
| **2. Import metrics (`data_bin`)** | Import only `data_bin` data between specific dates. | <CodeBlock language="shell">{`nohup /usr/share/centreon-bi/etl/importData.pl -r --no-purge --databin-only -s $date_start$ -e $date_end$ > /var/log/centreon-bi/rebuild_importDataBin.log &`}</CodeBlock> | A few minutes to several hours |
| **3. Update reporting dimensions** | Update the reporting dimensions. Use `-d` to preserve the history of configuration changes. | <CodeBlock language="shell">{`nohup /usr/share/centreon-bi/etl/dimensionsBuilder.pl -d > /var/log/centreon-bi/rebuild_dimensions.log &`}</CodeBlock> | **Fast** |
| **4. Rebuild events tables** | Rebuild events based on the retention period. | <CodeBlock language="shell">{`nohup /usr/share/centreon-bi/etl/eventStatisticsBuilder.pl -r --events-only --no-purge > /var/log/centreon-bi/rebuild_events.log &`}</CodeBlock> | **Several hours** |
| **5. Rebuild availability tables** | Rebuild availability stats between dates. | <CodeBlock language="shell">{`nohup /usr/share/centreon-bi/etl/eventStatisticsBuilder.pl -r --no-purge --availability-only -s $date_start$ -e $date_end$ > /var/log/centreon-bi/rebuild_availability.log &`}</CodeBlock> | **Several hours** |
| **6. Rebuild performance statistics** | Rebuild performance stats between dates. | <CodeBlock language="shell">{`nohup /usr/share/centreon-bi/etl/perfdataStatisticsBuilder.pl -r --no-purge -s $date_start$ -e $date_end$ > /var/log/centreon-bi/rebuild_perfData.log &`}</CodeBlock> | **Several hours** |
