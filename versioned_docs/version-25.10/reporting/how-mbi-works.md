...
2. On the central server, a script (**eventReportBuilder**) is launched by a cronjob every day at 3 AM: it converts check results into [events](https://docs-next-int.centreon.com/docs/reporting/concepts/#event) that will be used to calculate availability. The time this script is launched is defined in **/etc/cron.d/centreon**.
...
1. **/usr/share/centreon-bi/etl/importData.pl**: [Events](https://docs-next-int.centreon.com/docs/reporting/concepts/#event) and metrics as well as the configuration (hosts, host categories, ACLs...) are copied from the central database to the MBI database for the last day (from midnight to midnight).
...
2. **/usr/share/centreon-bi/etl/dimensionsBuilder.pl**: the ETL prepares a list of all [dimensions](https://docs-next-int.centreon.com/docs/reporting/concepts/#dimension) present in the imported data, for each host, service and metric.
...
Note that for [BAM](https://docs-next-int.centreon.com/docs/service-mapping/introduction/), aggregated data has already been computed by the central server. The ETL just copies it to the MBI server.
...
Example: Centreon can send reports by emails according to parameters defined in an SMTP [publication rule](reports-publication-rule.md) ([postfix](https://docs-next-int.centreon.com/docs/administration/postfix/) must be configured).
