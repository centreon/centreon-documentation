...
- [API] Fixed an issue with broker output type when creating an output using CLAPI.
- [API] Fixed an issue with password clearly displayed in Host & Service API response.
- [Authentication] Fixed authentication conditions that doesn't work with multiple values (SAML/OpenID Connect).
...
- [Authentication] Reduced logs messages for better understanding.
...
- [Install] Fixed SQL issue during update process.
- [Monitoring] Fixed display on Firefox when information contains single quote.
...
- [Resources Status] Fixed monitoring server filter when user belong to ACL.
- [UX] Fixed the closure of top menus in the banner after clicking on a link.
