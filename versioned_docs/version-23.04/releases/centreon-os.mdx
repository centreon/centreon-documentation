...
- [Install] Updated unattended.sh to manage updates, Debian 11 OS, langage packages.
...
- [Resources Status] Fixed pollers filter list by removing deleted pollers.
- [UI] Renamed "massive change" to "mass change" in "more actions" selectbox.
- [Monitoring] Fixed export of CSV when metric name is a SQL keywords.
- [Configuration] Fixed visibility of services when a user with ACL duplicate an host.
- [Authentication] Fixed LDAP connection issue due to DN case.
- [Resource Status] Fixed inline check action. Sends now a FORCED_CHECK.
