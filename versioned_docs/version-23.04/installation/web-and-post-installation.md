...
- **Root user/password**: this is the account that will be used to install the databases.
   - if this is the default account (**root**), the root password of the database is the password [you have defined](../installation/installation-of-a-central-server/using-packages.md#secure-the-database) when executing `mysql_secure_installation`.
