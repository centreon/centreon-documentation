...
Here are the bricks that make Centreon work. To find out on which port each brick communicates, see the [Tables of network flows](technical.md#tables-of-network-flows).
...
* Centreon Engine is present on each server in your [architecture](architectures.md): [pollers, remote servers and the central server](../getting-started/platform.md).
...
* Engine sends data to the master Broker:
   ...
   * The master Broker sends a copy of [metrics](../monitoring/metrics.md) and [statuses](../alerts-notifications/concepts.md) to the RRD Broker so that it can write them to the [RRD database](#a-rrd-databaserrd-files).
* The data is sent in [BBDO format](../developer/developer-broker-bbdo.md).
...
The MariaDB or MySQL database can be included in your central server, or [remote](architectures.md#architecture-with-remote-dbms).
...
3. The user [deploys the configuration](../monitoring/monitoring-servers/deploying-a-configuration.md): Gorgone takes the files that are generated and copies them to all the engines of the pollers and remote servers, and reloads these.
