...
- [Authentication] LDAP user with no mail can now authenticate on Centreon with LDAP auto import.
- [Configuration] Fixed a circular dependencies issue for service template inheritance (UI + API).
- [Configuration] Fixed a display issue on command information popin.
- [Configuration] Fixed an issue on "configuration changed" flag which was not updated when changing extended info using CLAPI.
- [Configuration] Fixed an issue where non explicit error message were displayed when saving host configuration if geo-coordinates are too long.
- [Configuration] Fixed host alias not properly handling quotes in input.
- [Configuration] Increased contact_pager property to enable longer tokens (e.g. Microsoft Teams)
...
- [Event Logs] Pagination was not working properly.
...
- [Resources Status] Fixed an issue on notification tab where same user was displayed multiple times.
...
- [Resource Status] Fixed notification tab to display the contact set on service template rather than on the host.
- [Resource Status] The pipe “|” character is now handled correctly in the command section of a resource.
- [Resource Status] View by host - Fixed issue preventing resources from being displayed when filtering on a resource that has no metric.
...
- [Vulnerability] Enhanced security on password DOM storage in login page.
