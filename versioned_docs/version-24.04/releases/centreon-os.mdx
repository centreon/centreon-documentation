...
- [Engine] fix(engine): output of external commands are sometimes truncated
- [Engine] fix (common): anomaly config files are open in readonly mode
- [Engine] fix (common): all logger configs are taken into account (runtime and otl)
...
- [Engine] If a service contains several times the same metric name, some of them were overwritten. Now during the parsing, a warning log explains the issue and only the first occurence of the metric is kept.
