...
- [Media][Image Synchronization] Added the handling of .svg files.
...
- [ACL] Fixed issue with hostgroup creation by a non admin user.
...
- [Configuration] Fixed an issue on remote Server where user loses contact group when edited.
- Fixed an issue where the outdated-token-removal cron wasn't working.
- [Image upload] Fixed a security vulnerability while uploading an image.
- [Media][Image Synchronization] Fixed issue with the case sensitivity of file extensions.
...
- [Resource status] Fixed an issue on host_category filter which doesn't work and returns an error.
