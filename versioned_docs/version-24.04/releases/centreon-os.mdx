...
- [API] Fixed an issue on configuration generation if an host have been patched with API.
- [Configuration] Fixed ACL check on the contact / user configuration page.
- [Configuration] Fixed an issue on notification period field which was not saved in some cases.
...
- [Downtimes] Fixed an issue on displaying the downtimes pages for users under ACLs.
- [Downtimes] Fixed an issue where loading downtimes page resulted in gateway timeout error.
- [Service groups] Fixed issue preventing non admin users from displaying Service Groups.
- [Top counter] Fixed an issue where calls were performed even if user hadn't access.
