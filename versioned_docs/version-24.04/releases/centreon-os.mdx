...
- [Broker] when no filter is configured, forbidden filter is applied to all. 
- [Engine] Fixed an issue regarding gRPC accepted services deletion.
- [Engine] When the Engine configuration is modified, no need to send host/service status since their configuration are already sent.
- [Engine] When Engine was restarted, its log file wasn't reopened, so if it was moved or removed, it wasn't created again.
...
- [Gorgone] Fixed an issue on cpu load when a poller was not responding.
- [Gorgone] Fixed an issue where Gorgone running on the central host was leaking file descriptor when poller disconnect.
- [Gorgone] Fixed nmap discovery provider not working after whitelist creation on Gorgone.
