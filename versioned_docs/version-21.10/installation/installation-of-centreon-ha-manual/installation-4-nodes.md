...
In order to allow servers to exchange files and commands, we need to implement the ability to connect *via* SSH from one server to another for users:
...
After running these commands on both nodes, copy the contents of the file that was displayed under the `cat` command and paste it into the file (to be created) `~/.ssh/authorized_keys`, on the other node and then apply the correct permissions to the file (still as `centreon`):
...
For the `mysql` account the procedure differs somewhat, as this user does not normally have a *home directory* or the ability to open a Shell session. This procedure is to be applied on both database nodes.
...
Once in the `bash` environment of `mysql`, run these commands on both database nodes:
...
The key exchange must then be validated by a first connection that will accept the signature of the SSH server (always as `mysql`) :
...
Some services should only be started on one node, but for others, it's okay or even desirable to start them on both core nodes. We will start with these services.
...
The VIP will be added to the `@CENTRAL_VIP_IFNAME@` interface configuration on both Centreon nodes:
...
This script can be launched from any DB node, this time we do not specify the name of the target server:
