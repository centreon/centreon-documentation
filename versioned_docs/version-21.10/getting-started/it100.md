...
* Make sure the email you provided in the IT 100 registration form is valid.
...
* Check whether a license is registered on the server. If a license is registered in the server, the following file will exist: **/etc/centreon/license.d/epp.license**.

* Check whether the fingerprint of the server is correct.

  Your license is linked to your server's fingerprint. In some cases, this fingerprint may change (e.g. when network interfaces have been modified).
  Retrieve the actual fingerprint of the server (go to **Administration > Extensions > Manager**, then click **Get fingerprint**) and compare it with the fingerprint that is registered in file **/etc/centreon/license.d/epp.license**. If they don't match:
    1. Request a new token with the correct fingerprint.
    2. Go to **Administration > Extensions > Manager** and then click **View license**, then **Unlink license**.
    3. Click **Upload license** to upload the new license.
