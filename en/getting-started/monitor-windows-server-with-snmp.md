...
Note that the state is : Not Present
...
Pay attention to the state. It must be Installed
...
A pop-in window appears and lists the criteria. Then select **service** in the field **Resource**. At first, the resources appear with the status **Pending**, which means that no checks have been executed yet:
