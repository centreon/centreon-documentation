...
* The **Address** field defines the IP address or DNS name of the host. The **Resolve** button enables us to
  resolve the domain name by questioning the DNS server configured on the SaaS platform. Note that for security reasons, parameters formated as `<protocol>://<ip-address>` are not accepted.
...
* **Custom macros**:
   ...
   * The **Name** and **Value** field enable us to define the name and value of the macro. Note that for security reasons, parameters formated as `<protocol>://<ip-address>` are not accepted.
