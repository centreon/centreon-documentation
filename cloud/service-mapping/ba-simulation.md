...
Use the simulation mode to validate the behavior of a Business Activity (BA) dependency tree before saving your changes and pushing the configuration to production.

You can simulate indicator statuses, adjust the values that influence status calculation, and decide afterward whether to keep or discard the changes you made.
...
Clicking the **Simulation mode** button switches the configuration page to a dedicated mode. The full dependency tree is displayed, and by default every last-level indicator (an indicator that has no children, or cannot have any) starts with the status OK.
